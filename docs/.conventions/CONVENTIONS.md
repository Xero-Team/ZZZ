# ZZZ Documentation Conventions

This document covers structural conventions for ZZZ documentation: what to document, how to organize it, and when to create new pages.
...
- **Version-specific behavior changes** — Include version callouts (e.g., "In ZZZ v0.224.0 and above...")
...
- Users would search for it **by name** (e.g., "ZZZ terminal", "ZZZ snippets")
...
- Commands: `:w`, `:q`
- File paths: `~/.config/zzz/settings.json`
- Action names: `git::Commit`
- Values: `true`, `false`, `"eager"`

### Action and Keybinding References

Use ZZZ's special syntax for dynamic rendering:
...
- Link to `zzz.dev` pages when appropriate
...
- For main feature pages with a matching marketing page, include a relevant
  `zzz.dev` marketing link in addition to docs links
...
Keep language docs focused on ZZZ-specific configuration, not general language tutorials.
...
> Configure inline blame in Settings ({#kb zzz::OpenSettings}) by searching for "inline blame", or add to your settings file:
>
> ```json [settings]
> {
>   "git": {
...
> Add the following to your settings file ([how to edit](./configuring-zzz.md#settings-files)):

### Settings File Locations

- **macOS/Linux:** `~/.config/zzz/settings.json`
...
- [ ] Main feature pages include a relevant `zzz.dev` marketing link
