...
The macOS releases labelled "Partially Supported" (Big Sur and Catalina)
do not support screen capture features that rely on the
[LiveKit SDK](https://livekit.io). That SDK depends on
[ScreenCaptureKit.framework](https://developer.apple.com/documentation/screencapturekit/),
which is only available on macOS 12 (Monterey) and newer.
