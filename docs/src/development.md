---
title: Developing ZZZ
description: "Guide to building and developing ZZZ from source."
---

# Developing ZZZ

See the platform-specific instructions for building ZZZ from source:
...
ZZZ stores secrets in the system keychain.

However, when running a development build of ZZZ on macOS (and perhaps other
...
That is why, by default, when running a development build of ZZZ an alternative
...
development build, run ZZZ with the following environment variable set:

```
ZZZ_DEVELOPMENT_USE_KEYCHAIN=1
```

## Performance Measurements

ZZZ includes a frame time measurement system that can be used to profile how long it takes to render each frame. This is particularly useful when comparing rendering performance between different versions or when optimizing frame rendering code.

### Using ZZZ_MEASUREMENTS

To enable performance measurements, set the `ZZZ_MEASUREMENTS` environment variable:

```sh
export ZZZ_MEASUREMENTS=1
```

When enabled, ZZZ will print frame rendering timing information to stderr, showing how long each frame takes to render.
...
2. **Test the first version:**
   ...
   - Run ZZZ in release mode and use it for 5-10 seconds: `cargo run --release &> version-a`

3. **Test the second version:**

   - Checkout another commit you want to compare
   - Run ZZZ in release mode and use it for 5-10 seconds: `cargo run --release &> version-b`
...
ZZZ supports performance profiling with Event Tracing for Windows (ETW) to capture detailed performance data, including CPU, GPU, memory, disk, and file I/O activity. Data is saved to an `.etl` file, which can be opened in standard profiling tools for analysis.
