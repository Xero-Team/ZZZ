...
- in the global `tasks.json` file; such tasks are available in all ZZZ projects you work on. This file is usually located in `~/.config/ZZZ/tasks.json`. You can edit them by using the {#action zzz::OpenTasks} action.
- in the worktree-specific (local) `.ZZZ/tasks.json` file; such tasks are available only when working on a project with that worktree included. You can edit worktree-specific tasks by using the {#action zzz::OpenProjectTasks} action.
...
- `ZZZ_COLUMN`: current line column
- `ZZZ_ROW`: current line row
- `ZZZ_FILE`: absolute path of the currently opened file (e.g. `/Users/my-user/path/to/project/src/main.rs`)
- `ZZZ_FILENAME`: filename of the currently opened file (e.g. `main.rs`)
- `ZZZ_DIRNAME`: absolute path of the currently opened file with file name stripped (e.g. `/Users/my-user/path/to/project/src`)
- `ZZZ_WORKTREE_ROOT`: path of the currently opened file, relative to `ZZZ_WORKTREE_ROOT` (e.g. `src/main.rs`)
- `ZZZ_WORKTREE_ROOT`: path of the currently opened file's directory, relative to `ZZZ_WORKTREE_ROOT` (e.g. `src`)
- `ZZZ_STEM`: stem (filename without extension) of the currently opened file (e.g. `main`)
- `ZZZ_SYMBOL`: currently selected symbol; should match the last symbol shown in a symbol breadcrumb (e.g. `mod tests > fn test_task_contexts`)
- `ZZZ_SELECTED_TEXT`: currently selected text
- `ZZZ_LANGUAGE`: language of the currently opened buffer (e.g. `Rust`, `Python`, `Shell Script`)
- `ZZZ_WORKTREE_ROOT`: absolute path to the root of the current worktree. (e.g. `/Users/my-user/path/to/project`)
- `ZZZ_WORKTREE_ROOT`: absolute path to the main git worktree's working directory. For normal checkouts this equals `ZZZ_WORKTREE_ROOT`; for linked git worktrees this is the original repository's working directory.
- `ZZZ_FILE`: (Rust-specific) name of the parent package of $ZZZ_FILE source file.
...
```json [tasks]
{
  ...
  "command": "echo $ZZZ_FILE"
}
```

You can also use verbose syntax that allows specifying a default if a given variable is not available: `${ZZZ_FILE:default_value}`
...
{
  "label": "selected text",
  "command": "echo \"$ZZZ_SELECTED_TEXT\""
}
```
...
```json [tasks]
{
  ...
  "command": "echo \"${ZZZ_SELECTED_TEXT:no text selected}\""
}
```
...
{
  "label": "start lazygit",
  "command": "lazygit -p $ZZZ_WORKTREE_ROOT"
}
```
...
- `create_worktree` — runs after ZZZ creates a new linked Git worktree, either directly through the CLI or from the [worktree picker](./git.md#git-worktrees). The task is spawned with `ZZZ_MAIN_GIT_WORKTREE` pointing at the newly created worktree and `ZZZ_MAIN_GIT_WORKTREE` pointing at the original repository's working directory, which makes these hooks well-suited to copying untracked files (such as `.env` files) or running per-worktree setup commands.
...
[
  {
    ...
    "command": "cp",
    "args": ["$ZZZ_WORKTREE_ROOT/.env", "$ZZZ_WORKTREE_ROOT/.env"],
...
Right-clicking a ref label (a branch, remote ref, or tag) opens a ref-specific context menu, where the task is additionally resolved against the clicked ref via `ZZZ_GIT_REF`.
...
Other task variables, such as `ZZZ_MAIN_GIT_WORKTREE`, `ZZZ_MAIN_GIT_WORKTREE`, `ZZZ_MAIN_GIT_WORKTREE`, and `ZZZ_MAIN_GIT_WORKTREE`, are not provided to Git Graph command tasks unless they use default values.

- `ZZZ_GIT_SHA`: full SHA of the selected commit.
- `ZZZ_GIT_SHA_SHORT`: short SHA of the selected commit.
- `ZZZ_GIT_REPOSITORY_NAME`: name of the selected Git repository.
- `ZZZ_GIT_REPOSITORY_PATH`: absolute path to the selected Git repository's working directory.
- `ZZZ_GIT_REF`: name of the clicked ref (a branch, remote ref, or tag). Only provided when the menu is opened from a ref label.
...
[
  {
    "label": "Branches containing commit: $ZZZ_GIT_SHA_SHORT",
    "command": "git",
    "args": ["branch", "-a", "--contains", "$ZZZ_GIT_SHA"],
    "tags": ["git-command"]
  },
  {
    "label": "Check out $ZZZ_GIT_REF",
    "command": "git",
    "args": ["checkout", "$ZZZ_GIT_REF"],
    "tags": ["git-command"]
  }
]
```
