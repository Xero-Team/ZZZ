---
title: Dev Containers - ZZZ
description: Open projects in dev containers with ZZZ. Reproducible development environments using devcontainer.json configuration.
---
...
If your repository includes a `.devcontainer/devcontainer.json` file, ZZZ can open a project inside a development container.
...
If you dismiss the prompt or want to reopen the project inside a container later, you can use ZZZ's command palette to run the "Project: Open Remote" command and select the option to open the project in a dev container.
...
If you modify `.devcontainer/devcontainer.json`, ZZZ does not currently rebuild or reload the container automatically. After changing configuration:
...
Once connected, ZZZ operates inside the container environment for tasks, terminals, and language servers.
...
- **Extensions:** ZZZ does not yet manage extensions separately for container environments. The host's extensions are used as-is.
