---
title: REPL - Jupyter Kernels in ZZZ
description: Run code interactively in ZZZ with built-in Jupyter kernel support. Execute Python, TypeScript, R, and more inline.
---
...
ZZZ's built-in REPL uses [Jupyter kernels](https://docs.jupyter.org/en/latest/projects/kernels.html) so you can run code interactively in regular editor files.

## Installation

ZZZ supports running code in multiple languages. To get started, you need to install a kernel for the language you want to use.
...
ZZZ supports [notebooks as scripts](https://jupytext.readthedocs.io/en/latest/formats-scripts.html) using the `# %%` cell separator in Python and `// %%` in TypeScript. This allows you to write code in a single file and run it as if it were a notebook, cell by cell.
...
> **Note:** On macOS, your system Python will _not_ work. Either set up
> [pyenv](https://github.com/pyenv/pyenv?tab=readme-ov-file#installation) or
> use a virtual environment. ZZZ uses separate selections for the Python
> [toolchain](./configuring-languages.md#toolchains) and REPL kernel. If
> diagnostics differ from the REPL, align the toolchain with the kernel using
> {#action toolchain::Select}. Read more about the [toolchain selector for
> Python](./languages/python.md#selecting-a-toolchain).
...
- Install the R Extension for ZZZ (search for `R` in ZZZ Extensions)
...
- Install the Julia Extension for ZZZ (search for `Julia` in ZZZ Extensions)
...
ZZZ automatically detects available kernels and organizes them in the kernel picker:
...
Python environments appear in the picker even if ipykernel is not installed. Environments missing ipykernel are dimmed and labeled "ipykernel not installed." When you select one, ZZZ automatically runs `pip install ipykernel` in that environment and activates it once installation completes.

### How ZZZ Recommends Kernels

When you run code, ZZZ selects a kernel automatically:

1. **Active toolchain match**: If a Python environment matches your active toolchain and has ipykernel, ZZZ uses it
2. **First available Python env**: Otherwise, the first Python environment with ipykernel
3. **Language-based fallback**: If no Python envs are ready, ZZZ picks a Jupyter kernel matching the code block's language
...
> Note: ZZZ makes best effort usage of `sys.prefix` and `CONDA_PREFIX` to find kernels in Python environments. If you want explicitly control run `python -m ipykernel install --user --name myenv --display-name "Python (myenv)"` to install the kernel directly while in the environment.
