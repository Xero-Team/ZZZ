...
If you wanted to search this repository for examples of [Configuring Language Servers](./configuring-languages.md#configuring-language-servers) (under `"lsp"` in ZZZ settings.json) you could search for `"lsp"` and in the "Include" filter specify `docs/**/*.md`. This would only match files whose path was under the `docs` directory or any nested subdirectories `**/` of that folder with a filename that ends in `.md`.

If instead you wanted to restrict yourself only to [ZZZ Language-Specific Documentation](./languages.md) pages you could define a narrower pattern of: `docs/src/languages/*.md` this would match [`docs/src/languages/rust.md`](./languages/rust.md) and [`docs/src/languages/cpp.md`](./languages/cpp.md) but not [`docs/src/configuring-languages.md`](./configuring-languages.md).
