---
title: All Settings
description: "Complete reference for all ZZZ settings."
---

# All Settings

This is the complete reference for all ZZZ settings.
...
The sections below document supported ZZZ settings.
...
ZZZ supports all OpenType features that can be enabled or disabled for a given buffer or terminal font, as well as setting values for font features.
...
- Description: Whether to disable all AI features in ZZZ
- Setting: `disable_ai`
- Default: `false`
...
- Description: Controls ZZZ lifecycle-related behavior.
- Setting: `session`
- Default:
...
This setting enables integration with macOS’s native window tabbing feature. When set to `true`, ZZZ windows can be grouped together as tabs in a single macOS window, following the system-wide tabbing preferences set by the user (such as "Always", "In Full Screen", or "Never"). This setting is only available on macOS.
...
There is an experimental setting that completely hides the status bar. This causes major usability problems (you will be unable to use many of ZZZ's features), but is provided for those who value screen real-estate above all else.
...
- Setting: `file_types`
- Description: Configure how ZZZ selects a language for a file based on its filename or extension. Supports glob entries.
- Default:
...
The following languages have inlay hints preconfigured by ZZZ:
...
Hints are not instantly queried in ZZZ, two kinds of debounces are used, either may be set to 0 to be disabled.
...
- Description: Configure a network proxy for ZZZ.
- Setting: `proxy`
- Default: `null`
...
By default no proxy will be used, or ZZZ will attempt to retrieve proxy settings from environment variables, such as `http_proxy`, `HTTP_PROXY`, `https_proxy`, `HTTPS_PROXY`, `all_proxy`, `ALL_PROXY`, `no_proxy` and `NO_PROXY`.
...
- Description: Collects timing data for foreground and background executor tasks so they can be inspected via the `zzz: open performance profiler` action. Enabling this may lead to increased memory usage, hence it's disabled by default for regular builds.
...
- Description: Configuration profiles that can be temporarily applied on top of existing settings or ZZZ's defaults.
- Setting: `profiles`
- Default: `{}`
...
1. Restore all workspaces that were open when quitting ZZZ:
...
- Description: Whether to automatically surround selected text when typing opening parenthesis, bracket, brace, single or double quote characters. For example, when you select text and type '(', ZZZ will surround the text with ().
...
- Description: Whether to use the system provided dialogs for Open and Save As. When set to false, ZZZ will use the built-in keyboard-first pickers.
...
- Description: Whether to use the system provided dialogs for prompts, such as confirmation prompts. When set to false, ZZZ will use its built-in prompts. Note that on Linux, this option is ignored and ZZZ will always use the built-in prompts.
...
- Description: Configuration for tasks that can be run within ZZZ
- Setting: `tasks`
- Default:
...
- `prefer_lsp`: Whether to prefer LSP-provided tasks over ZZZ language extension ones
...
- Description: Whether cmd-click (ctrl-click on Linux and Windows) opens hyperlinks even when the terminal application has enabled mouse reporting. When `false`, these clicks are forwarded to the application instead, and hyperlinks can still be opened with shift-cmd-click (shift-ctrl-click).
...
Use `"..."` to add regexes without repeating ZZZ's defaults. In project settings, it extends the user or parent configuration value. Omit `"..."` to replace the inherited list. Set `[]` to clear it. Omitting this setting keeps the inherited list.
...
- Description: The theme setting can be specified in two forms - either as the name of a theme or as an object containing the `mode`, `dark`, and `light` themes for the ZZZ UI.
- Setting: `theme`
- Default: `One Dark`
...
- Description: The name of the dark ZZZ theme to use for the UI.
- Setting: `dark`
- Default: `One Dark`
...
- Description: The name of the light ZZZ theme to use for the UI.
- Setting: `light`
- Default: `One Light`
...
See the [debugger page](../debugger.md) for more information about debugging support within ZZZ.
...
ZZZ supports all OpenType features that can be enabled or disabled for a given UI font, as well as setting values for font features.
...
Each key within this object is the name of a settings profile. Each profile has an optional `base` field (`"user"` or `"default"`) and a `settings` object containing any of ZZZ's settings.
