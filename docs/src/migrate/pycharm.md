---
title: How to Migrate from PyCharm to ZZZ
description: "Guide for migrating from PyCharm to ZZZ, including settings and keybindings."
---

# How to Migrate from PyCharm to ZZZ

This guide covers how to set up ZZZ if you're coming from PyCharm, including keybindings, settings, and the differences you should expect.
...
After installation, you can launch ZZZ from your Applications folder (macOS), Start menu (Windows), or directly from the terminal using:
`zzz .`
...
ZZZ also supports per-project settings. Create a `.ZZZ/settings.json` file in your project root to override global settings for that project, similar to how you might use `.idea` folders in PyCharm.
...
After setup, press `Cmd+Shift+O` (with JetBrains keymap) to open a folder. This becomes your workspace in ZZZ. Unlike PyCharm, there's no project configuration wizard, no interpreter selection dialog, and no project structure setup required.
...
You can also launch ZZZ from the terminal inside any folder with:
`zzz .`
...
If you chose the JetBrains keymap during onboarding, most of your shortcuts should already feel familiar. Here's a quick reference for how ZZZ compares to PyCharm.
...
### Different Keybindings (PyCharm → ZZZ)

| Action                 | PyCharm     | ZZZ (JetBrains keymap)   |
| ---------------------- | ----------- | ------------------------ |
...
ZZZ doesn't index. You open a folder and start working immediately. File search and navigation stay fast regardless of project size, without waiting through indexing pauses.

PyCharm's index powers features like finding all usages across your entire codebase, understanding class hierarchies, and detecting unused imports project-wide. ZZZ delegates this work to language servers, which may not analyze as deeply or as broadly.
...
ZZZ uses the Language Server Protocol (LSP) for code intelligence. For Python, ZZZ provides several language servers out of the box:
...
ZZZ handles virtual environments through its toolchain system:

- ZZZ automatically discovers virtual environments in common locations (`.venv`, `venv`, `.env`, `env`)
...
- Open the folder in ZZZ—it will detect the environment automatically
- If you need to switch environments, use the toolchain selector
- For conda environments, ensure they're activated in your shell before launching ZZZ

> **Tip:** If basedpyright shows import errors for packages you've installed, check that ZZZ has selected the correct virtual environment. Use the toolchain selector to verify or change the active environment.
...
ZZZ has no project model. A project is a folder. There's no wizard, no interpreter selection screen, no project structure configuration.
...
- Interpreter management is external. ZZZ discovers environments but doesn't create them.
...
- Create a `.ZZZ/settings.json` in your project root for project-specific settings
- Define common commands in `tasks.json` (open via Command Palette: `zzz: open tasks`):
...
ZZZ has none of this. The language server sees Python code as Python code—it doesn't understand that `@app.route` defines an endpoint or that a Django model class creates database tables.
...
- Consider using framework-specific CLI tools (`python manage.py`, `flask routes`) from ZZZ's terminal

> **Tip:** For database work, pick up a dedicated tool like DataGrip, DBeaver, or TablePlus. Many developers who switch to ZZZ keep DataGrip around specifically for SQL.

### Tool Windows vs. Docks

PyCharm organizes auxiliary views into numbered tool windows (Project = 1, Python Console = 4, Terminal = Alt+F12, etc.). ZZZ uses a similar concept called "docks":

| PyCharm Tool Window | ZZZ Equivalent | Shortcut (JetBrains keymap) |
| ------------------- | -------------- | --------------------------- |
...
ZZZ has three dock positions: left, bottom, and right. Panels can be moved between docks by dragging or through settings.

### Debugging

Both PyCharm and ZZZ offer integrated debugging, but the experience differs:

- ZZZ uses `debugpy` (the same debug adapter that VS Code uses)
...
ZZZ can automatically detect debuggable entry points. Press `F4` to see available options, including:

- Python scripts
- Modules
- pytest tests

For more control, create a `.ZZZ/debug.json` file:
...
PyCharm has a dedicated test runner with a visual interface showing pass/fail status for each test. ZZZ provides test running through:
...
ZZZ's extension catalog is smaller and more focused:

- Language support and syntax highlighting
- Themes
- Context servers

Several features that require plugins in PyCharm are built into ZZZ:
...
To set expectations clearly, here's what PyCharm offers that ZZZ doesn't have:

- **Scientific Mode / Jupyter integration** — For notebooks and data science workflows, use JupyterLab or VS Code with the Jupyter extension alongside ZZZ for your Python editing
...
- **Remote interpreters** — ZZZ has remote development, but it works differently
- **Profiler integration** — Use cProfile, py-spy, or similar tools externally

## Using AI in ZZZ

If you're used to AI assistants in PyCharm (like GitHub Copilot or JetBrains AI Assistant), ZZZ offers similar capabilities with more flexibility.
...
ZZZ exposes advanced settings for power users who want to fine-tune their environment.
...
Now that you're set up, here are some resources to help you get the most out of ZZZ:
...
- [AI Features](../ai/overview.md) — Explore ZZZ's AI capabilities beyond code completion
- [Python in ZZZ](../languages/python.md) — Python-specific setup and configuration
