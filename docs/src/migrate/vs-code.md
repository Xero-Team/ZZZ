---
title: How to Migrate from VS Code to ZZZ
description: "Guide for migrating from VS Code to ZZZ, including settings and keybindings."
---

# How to Migrate from VS Code to ZZZ

This guide explains how to move from VS Code to ZZZ without rebuilding your workflow.
...
During setup, you have the option to import key settings from VS Code. ZZZ imports the following settings:
...
ZZZ doesn’t import extensions or keybindings, but this import gets core editor behavior close to your VS Code setup. If you skip that step during setup, you can still import settings manually later via the command palette:

`Cmd+Shift+P → ZZZ: Import VS Code Settings`
...
ZZZ also supports per-project settings. You can find these in the Settings Editor as well.
...
### Common Shared Keybindings (ZZZ <> VS Code)
...
### Different Keybindings (ZZZ <> VS Code)
...
ZZZ also supports chords (multi-key sequences) like `Cmd+K Cmd+C`, like VS Code does.
...
ZZZ takes a different approach.
...
- **Per-project settings are optional**: You can add a `.ZZZ/settings.json` file inside a project to override global settings.

- **You can start from a single file or an empty window**: ZZZ doesn’t require you to open a folder to begin editing.
...
- Instead of a persistent panel, ZZZ encourages you to:
...
ZZZ does not offer as many extensions as VS Code. The available extensions are focused on language support, themes, syntax highlighting, and other core editing enhancements.

Several features that typically require extensions in VS Code are built into ZZZ:
...
You won’t find one-to-one replacements for every VS Code extension, especially if you rely on tools for DevOps, containers, or test runners. ZZZ's extension catalog is still growing and remains smaller.
...
**Custom Tasks**: Define build or run commands in your `tasks.json` (accessed via command palette: `zzz: open tasks`):
...
Copy your VS Code snippet JSON directly into ZZZ's snippets folder (`zzz: configure snippets`).
