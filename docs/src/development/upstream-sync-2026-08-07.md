...
| 95106f9c | B     | --           | Docs await final feature-flag disposition.                        |
| a8b57a25 | B     | --           | Transient undo feature flag requires local release policy review. |
| 1efdc3e6 | B     | --           | LSP response-first fix needs task lifetime adaptation.            |
| fa1d0362 | A     | b2206202     | Cherry-picked with `-x -s`.                                       |
| fee527c7 | B     | --           | Markdown rendering test/API drift needs a local port.             |
| 945764f9 | A     | b6b9c9dc     | Cherry-picked with `-x -s`.                                       |
| b2131e9d | B     | --           | Web worker fetch depends on diverged GPUI Web dispatch API.       |
| b6ebe0ff | A     | d67e88ac     | Cherry-picked with `-x -s`.                                       |
| 1102219f | B     | --           | Overlaps ZZZ's local Markdown-link resolver port.                 |
