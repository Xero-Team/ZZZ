...
If you set the `RUSTFLAGS` env var, it will override the `rustflags` settings in `.cargo/config.toml` which is required to properly build ZZZ.
...
Or, create a new `.cargo/config.toml` in the parent directory of the ZZZ repo (see below). This is useful in CI because you do not need to edit the repo's original `.cargo/config.toml`.
...
- `ZZZ failed to open a window: NoSupportedDeviceFound`
...
If there's nothing Vulkan-related in the logs and you happen to have Bandicam installed, try uninstalling it. ZZZ is currently not compatible with Bandicam.
