# ZZZ's Feature Development Process
...
**Decisions:**
We have to decide whether to use the git CLI or a git library. ZZZ uses a git library but its blame implementation is too slow for a code editor, so we should use the CLI's porcelain interface.
...
- **Security.** How does this feature interact with Workspace Trust? Does it open new attack surfaces in ZZZ?
