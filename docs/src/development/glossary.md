...
This page defines terms and structures used throughout the ZZZ codebase.
...
- `Window`: A struct representing a ZZZ window in your desktop environment (see image below). You can have multiple windows open. This is mostly passed around for rendering.
...
- [Multibuffer](https://zed.dev/docs/multibuffers): A list of Editors, a multi-buffer allows editing multiple files simultaneously. A multi-buffer opens when an operation in ZZZ returns multiple locations, examples: _search_ or _go to definition_. See project search in the image below.
...
- `Editor`: The text editor type. Most editable surfaces in ZZZ are an `Editor`, including single-line inputs. Each pane in the image above contains one or more `Editor` instances.
...
- `BreakpointStore`: Is an entity that manages breakpoints states in local and remote instances of ZZZ
