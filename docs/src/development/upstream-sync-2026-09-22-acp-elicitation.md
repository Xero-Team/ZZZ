...
- Target branch: `sync/upstream-2026-09-22-acp-elicitation` from `main` at
  `5acf32ed6226bc39999ef8cb81b744d7483ee31b`
...
| ---------- | ----- | ------------ | ------------------------------------------------------------------------------------------------- |
| c413552859 | B     | 6d6c8dbd02   | SDK 0.13.1 migration; drop the session-model selector replaced by session config options.         |
| 56b71271c4 | B     | 6d6c8dbd02   | SDK 0.14.0; session usage/deletion already backported locally, only the version bump is retained. |
| 8ba35e5eac | B     | 6d6c8dbd02   | SDK 0.15.0 API migration across `acp_thread`, `agent_servers`, and `agent_ui`.                    |
| 9de0590a81 | B     | 6d6c8dbd02   | SDK 1.0.0.                                                                                        |
| b04ae5ed81 | B     | 6d6c8dbd02   | SDK 1.0.1.                                                                                        |
| f15a3ef452 | B     | 6d6c8dbd02   | SDK 1.3.0.                                                                                        |
| 984e3bd0ce | B     | 6d6c8dbd02   | SDK 2.0.0.                                                                                        |
| 3cef31688a | B     | 6d6c8dbd02   | SDK 2.1.0; the `futures` bump was not needed.                                                     |
| fa66442a01 | B     | 3f297b3c05   | Elicitation store, client handlers, capability, and session-scoped cards.                         |
| c05b439174 | B     | 3f297b3c05   | Elicitation option descriptions.                                                                  |
| fbceb2823b | B     | 3f297b3c05   | Enable ACP elicitations.                                                                          |
| 33e2058302 | B     | 3f297b3c05   | Elicitation warnings and validation.                                                              |
| 68ec865bfd | B     | 3f297b3c05   | Elicitation form keyboard navigation.                                                             |
| 8f92822cbf | B     | 3f297b3c05   | Only the `unicode_confusables` scanner needed by the elicitation URL warning is retained.         |
| 632d805d64 | C     | --           | Native agent `ask_user`; ZZZ keeps no native agent surface.                                       |
| 73ee8fa038 | C     | --           | Native agent `ask_user` option wrapping.                                                          |

## Applied work

### 6d6c8dbd02 — `sync: update agent-client-protocol SDK to 2.1.0`
...
### 3f297b3c05 — `sync: add ACP elicitation support from fa66442a01`
