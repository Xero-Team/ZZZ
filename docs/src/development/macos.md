...
ZZZ includes visual regression tests that capture screenshots of real ZZZ windows and compare them against baseline images. These tests require macOS with Screen Recording permission.
...
If ZZZ continually rebuilds root crates, you may be opening the ZZZ codebase itself in your development build.
...
build of ZZZ. These environment variables are in turn passed to `cargo check`, which
...
If you build ZZZ frequently, macOS may keep verifying new builds, which can add a few seconds to each iteration.
