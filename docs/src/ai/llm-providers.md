---
title: LLM Providers - Use Your Own API Keys in ZZZ
description: Bring your own API keys to ZZZ. Set up Anthropic, OpenAI, Google AI, Ollama, DeepSeek, Mistral, OpenRouter, Vercel AI Gateway, and more.
---
...
If you already have an API key for a provider like Anthropic or OpenAI, you can add it to ZZZ. No ZZZ subscription required.
...
ZZZ supports these providers with your own API keys:
...
2. Open your settings file (`zzz: open settings file`) and include the `bedrock` key under `language_models` with the following settings:
   ```json [settings]
   {
     "language_models": {
       "bedrock": {
...
The ZZZ implementation of Amazon Bedrock uses [Cross-Region inference](https://docs.aws.amazon.com/bedrock/latest/userguide/cross-region-inference.html) to improve availability and throughput.
...
By default, ZZZ uses **regional profiles** which keep your data within the same geography. You can opt into global profiles by adding `"allow_global": true` to your Bedrock configuration:
...
ZZZ will also use the `ANTHROPIC_API_KEY` environment variable if it's defined.
...
ZZZ will also use the `DEEPSEEK_API_KEY` environment variable if it's defined.
...
To use Copilot Enterprise with ZZZ (for both agent and completions), you must configure your enterprise endpoint as described in [Configuring GitHub Copilot Enterprise](./edit-prediction.md#using-github-copilot-enterprise).

### Google AI {#google-ai}

You can use Gemini models with the ZZZ agent by choosing it via the model dropdown in the Agent Panel.
...
ZZZ will also use the `GEMINI_API_KEY` environment variable if it's defined. See [Using Gemini API keys](https://ai.google.dev/gemini-api/docs/api-key) in the Gemini docs for more.
...
ZZZ will also use the `MISTRAL_API_KEY` environment variable if it's defined.
...
ZZZ will automatically discover models that Ollama has pulled. You can turn this off by setting
...
ZZZ API requests to Ollama include the context length as the `num_ctx` parameter. By default, ZZZ uses a context length of `4096` tokens for all Ollama models.
...
If the model is tagged with `thinking` in the Ollama catalog, set this option and you can use it in ZZZ.
...
ZZZ will also use the `OPENAI_API_KEY` environment variable if it's defined.
...
If a model does not support the `/chat/completions` endpoint (for example `gpt-5.2-codex`), disable it by setting `capabilities.chat_completions` to `false`. ZZZ will use the Responses endpoint instead.

Custom models will be listed in the model dropdown in the Agent Panel.

### OpenAI API Compatible {#openai-api-compatible}

ZZZ supports using [OpenAI compatible APIs](https://platform.openai.com/docs/api-reference/chat) by specifying a custom `api_url` and `available_models` for the OpenAI provider.
...
If a provider exposes models that only work with the Responses API, set `chat_completions` to `false` for those entries. ZZZ uses the Responses endpoint for these models.
...
ZZZ will also use the `OPENROUTER_API_KEY` environment variable if it's defined.
...
ZZZ will also use the `VERCEL_AI_GATEWAY_API_KEY` environment variable if it's defined.
...
ZZZ includes a dedicated [xAI](https://x.ai/) provider. You can use your own API key to access Grok models.
...
The xAI API key will be saved in your keychain. ZZZ will also use the `XAI_API_KEY` environment variable if it's defined.

> **Note:** The xAI API is OpenAI-compatible, and ZZZ also includes a dedicated xAI provider. We recommend using the dedicated `x_ai` provider configuration instead of the [OpenAI API Compatible](#openai-api-compatible) method.
