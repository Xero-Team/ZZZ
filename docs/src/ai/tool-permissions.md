...
Use ZZZ's Settings Editor to [configure tool permissions](zzz://settings/agent.tool_permissions), or add rules directly to your settings file:
...
This bypasses confirmation prompts for most tools, but `always_deny`, `always_confirm`, built-in security rules, and paths inside ZZZ settings directories still prompt or block.

## Shell Compatibility

For the `terminal` tool, ZZZ parses chained commands (e.g., `echo hello && rm file`) to check each sub-command against your patterns.
...
ZZZ includes a small set of hardcoded security rules that **cannot be overridden** by any setting.
