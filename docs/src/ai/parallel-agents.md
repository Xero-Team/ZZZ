...
To automate setup steps whenever a new worktree is created use a [Task hook](../tasks.md#hooks). The `create_worktree` hook runs automatically after ZZZ creates a linked worktree, with `ZED_MAIN_GIT_WORKTREE` pointing at the new worktree and `ZED_MAIN_GIT_WORKTREE` pointing at the original repository.
