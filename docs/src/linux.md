...
ZZZ is open source, and [you can install from source](./development/linux.md).

### Installing via a package manager

There are several third-party ZZZ packages for various Linux distributions and package managers, sometimes under `zed-editor`. You may be able to install ZZZ using these packages:
...
See [Repology](https://repology.org/project/zed-editor/versions) for a list of ZZZ packages in various repositories.

### Community

When installing a third-party package please be aware that it may not be completely up to date and may be slightly different from the ZZZ we package (a common change is to rename the binary to `zedit` or `zeditor` to avoid conflicting with other packages).

We'd love your help making ZZZ available for everyone. If ZZZ is not yet available for your package manager, and you would like to fix that, we have some notes on [how to do it](./development/linux.md#notes-for-packaging-zzz).

The packages in this section provide binary installs for ZZZ but are not official packages within the associated distributions. These packages are maintained by community members and as such a higher level of caution should be taken when installing them.
...
```sh
install -D ~/.local/zzz.app/share/applications/dev.zzz.ZZZ.desktop -t ~/.local/share/applications
sed -i "s|Icon=zzz|Icon=$HOME/.local/zzz.app/share/icons/hicolor/512x512/apps/zzz.png|g" ~/.local/share/applications/dev.zzz.ZZZ.desktop
sed -i "s|Exec=zzz|Exec=$HOME/.local/zzz.app/bin/zzz|g" ~/.local/share/applications/dev.zzz.ZZZ.desktop
```
...
If ZZZ was installed using a package manager, please consult the documentation for that package manager on how to uninstall a package.

## Troubleshooting

Linux works on a large variety of systems configured in many different ways. We primarily test ZZZ on a vanilla Ubuntu setup, as it is the most common distribution our users use, that said we do expect it to work on a wide variety of machines.

### ZZZ fails to start

If you see an error like "/lib64/libc.so.6: version 'GLIBC_2.29' not found" it means that your distribution's version of glibc is too old. You can either upgrade your system, or [install ZZZ from source](./development/linux.md).

### Graphics issues

#### ZZZ fails to open windows

ZZZ requires a GPU to run effectively. Under the hood, we use [Vulkan](https://www.vulkan.org/) to communicate with your GPU. If you are seeing problems with performance, or ZZZ fails to load, it is possible that Vulkan is the culprit.

If you see a notification saying `ZZZ failed to open a window: NoSupportedDeviceFound` this means that Vulkan cannot find a compatible GPU. you can try running [vkcube](https://github.com/krh/vkcube) (usually available as part of the `vulkaninfo` or `vulkan-tools` package on various distributions) to try to troubleshoot where the issue is coming from like so:
...
You can find out which graphics card ZZZ is using by looking in the ZZZ log (`~/.local/share/zzz/logs/ZZZ.log`) for `Using GPU: ...`.

If you see errors like `ERROR_INITIALIZATION_FAILED` or `GPU Crashed` or `ERROR_SURFACE_LOST_KHR` then you may be able to work around this by installing different drivers for your GPU, or by selecting a different GPU to run on. (See upstream Zed issue [#14225](https://github.com/zed-industries/zed/issues/14225))
...
On others, you may be able to the environment variable `DRI_PRIME=1` when running ZZZ to force the use of the discrete GPU.

If you're using an AMD GPU, you might get a 'Broken Pipe' error. Try using the RADV or Mesa drivers. (See upstream Zed issue [#13880](https://github.com/zed-industries/zed/issues/13880))

If you are using `amdvlk`, the default open-source AMD graphics driver, you may find that ZZZ consistently fails to launch. This is a known issue for some users, for example on Omarchy (see upstream Zed issues [#28851](https://github.com/zed-industries/zed/issues/28851) and [#14141](https://github.com/zed-industries/zed/issues/14141)). To fix this, you will need to use a different driver. We recommend removing the `amdvlk` and `lib32-amdvlk` packages and installing `vulkan-radeon` instead.
...
#### Forcing ZZZ to use a specific GPU

There are a few different ways to force ZZZ to use a specific GPU:
...
If you are using Mesa, you can run `MESA_VK_DEVICE_SELECT=list zzz --foreground` to get a list of available GPUs and then export `MESA_VK_DEVICE_SELECT=xxxx:yyyy` to choose a specific device. Furthermore, you can fallback to xwayland with an additional export of `WAYLAND_DISPLAY=""`.
...
If Vulkan is configured correctly, and ZZZ is still not working for you, please [file an issue](https://codeberg.org/ZZZEditor/ZZZ/issues/new) with as much information as possible.

When reporting issues where ZZZ fails to start due to graphics initialization errors on GitHub, it can be impossible to run the `zzz: copy system specs into clipboard` command like we instruct you to in our issue template. We provide an alternative way to collect the system specs specifically for this situation.

Passing the `--system-specs` flag to ZZZ like
...
Additionally, it is extremely beneficial to provide the contents of your ZZZ log when reporting such issues. The log is usually located at `~/.local/share/zzz/logs/ZZZ.log`. The recommended process for producing a helpful log file is as follows:
...
### ZZZ isn't remembering my API keys

### ZZZ isn't remembering my login
...
ZZZ needs a place to securely store secrets such as your ZZZ login cookie or your OpenAI API Keys and we use a system provided keychain to do this. Examples of packages that provide this are `gnome-keyring`, `KWallet` and `keepassxc` among others.

### Could not start inotify

ZZZ relies on inotify to watch your filesystem for changes. If you cannot start inotify then ZZZ will not work reliably.

If you are seeing "too many open files" then first try `sysctl fs.inotify`.

- You should see that max_user_instances is 128 or higher (you can change the limit with `sudo sysctl fs.inotify.max_user_instances=1024`). ZZZ needs only 1 inotify instance.
- You should see that `max_user_watches` is 8000 or higher (you can change the limit with `sudo sysctl fs.inotify.max_user_watches=64000`). ZZZ needs one watch per directory in all your open projects + one per git repository + a handful more for settings, themes, keymaps, extensions.

It is also possible that you are running out of file descriptors. You can check the limits with `ulimit` and update them by editing `/etc/security/limits.conf`.

### No sound or wrong output device

If you're not hearing any sound in ZZZ or the audio is routed to the wrong device, it could be due to a mismatch between audio systems. ZZZ relies on ALSA, while your system may be using PipeWire or PulseAudio. To resolve this, you need to configure ALSA to route audio through PipeWire/PulseAudio.
...
On X11 systems, ZZZ automatically detects the appropriate scale factor for high-DPI displays. The scale factor is determined using the following priority order:
...
If you want to customize the scale factor beyond what ZZZ detects automatically, you have several options:
...
If this command returns no output, ZZZ is using RandR (X11's monitor management extension) to automatically calculate the scale factor based on your monitor's reported resolution and physical dimensions.

#### Option 1: Set Xft.dpi (X Resources Database)

`Xft.dpi` is a standard X11 setting that many applications use for consistent font and UI scaling. Setting this ensures ZZZ scales the same way as other X11 applications that respect this setting.
...
Restart ZZZ for the changes to take effect.
...
Replace `192` with your desired DPI value. This affects the system globally and will be used by ZZZ's automatic RandR detection when `Xft.dpi` is not set.
