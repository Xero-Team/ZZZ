---
title: Themes - ZZZ
description: Browse, install, and create themes for ZZZ. Includes built-in themes and community theme extensions.
---

# Themes

ZZZ comes with a number of built-in themes, with more themes available as extensions.
...
For example, [assets/themes/one/one.json](../../assets/themes/one/one.json) for the default One Dark and One Light themes.

## Local Themes {#local-themes}

Store new themes locally by placing them in the `~/.config/ZZZ/themes` directory (macOS and Linux) or `%USERPROFILE%\AppData\Roaming\ZZZ\themes\` (Windows).

For example, to create a new theme called `my-cool-theme`, create a file called `my-cool-theme.json` in that directory.
It will be available in the theme selector the next time ZZZ loads.
