---
title: Ansible
description: "Configure Ansible language support in ZZZ, including language servers, formatting, and debugging."
---

# Ansible

Support for Ansible in ZZZ is provided via a community-maintained [Ansible extension](https://github.com/kartikvashistha/zed-ansible).
...
To change this behavior, you can add a `"file_types"` section to ZZZ settings inside your project (`.ZZZ/settings.json`) or your ZZZ user settings (`~/.config/ZZZ/settings.json`) to match your folder/naming conventions. For example:
...
- or, configure the YAML language server settings to set this schema for all your inventory files, that match your inventory pattern, under your ZZZ settings ([ref](https://zed.dev/docs/languages/yaml)):
...
When desired, any of the above default settings can be overridden under the `"lsp"` section of your ZZZ settings file. For example:

```json [settings]
{
  "lsp": {
    // The ZZZ Ansible extension prefixes all settings with `ansible`
