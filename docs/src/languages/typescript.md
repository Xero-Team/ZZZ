---
title: TypeScript
description: "Configure TypeScript language support in ZZZ, including language servers, formatting, and debugging."
---

# TypeScript

TypeScript and TSX support are available natively in ZZZ.
...
ZZZ sets the following initialization options to make the language server send back inlay hints (that is, when ZZZ has inlay hints enabled in the settings).

You can override these settings in your ZZZ `settings.json` when using `typescript-language-server`:
...
ZZZ enables references and implementations code lenses for `vtsls` by default. These show reference counts and implementation counts above functions, classes, and interfaces. To use them, enable the `code_lens` setting:
...
ZZZ supports debugging TypeScript code out of the box with `vscode-js-debug`.
...
As for all languages, configurations from `.vscode/launch.json` are also available for debugging in ZZZ.

If your use-case isn't covered by any of these, you can take full control by adding debug configurations to `.ZZZ/debug.json`. See below for example configurations.
...
- [ZZZ Yarn documentation](./yarn.md) for a walkthrough of configuring your project to use Yarn.
- [ZZZ Deno documentation](./deno.md)
