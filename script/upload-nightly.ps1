...
$remoteServerFiles = Get-ChildItem -Path "target" -Filter "zzz-remote-server-windows-*.zip" -Recurse -File -ErrorAction SilentlyContinue
...
UploadToBlobStore -BucketName $bucketName -FileToUpload "target/ZZZ-$Architecture.exe" -BlobStoreKey "nightly/ZZZ-$Architecture.exe"
UploadToBlobStore -BucketName $bucketName -FileToUpload "target/ZZZ-$Architecture.exe" -BlobStoreKey "$version/ZZZ-$Architecture.exe"

Remove-Item -Path "target/ZZZ-$Architecture.exe" -ErrorAction SilentlyContinue
