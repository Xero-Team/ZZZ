...
test_multi_channel_and_xdg_isolation() {
    ...
    : > "$zzz_data/zzz-stable.sock"
    : > "$zzz_data/zzz-dev.sock"
    ...
    assert_exists "$zzz_data/zzz-stable.sock"
...
test_dev_scope_isolated_from_stable() {
    ...
    assert_missing "$zzz_data/zzz-dev.sock"
...
test_unknown_channel_does_not_delete_data() {
    ...
    if HOME="$test_home" XDG_DATA_HOME="$test_data_home" \
        XDG_CONFIG_HOME="$test_config_home" ZZZ_CHANNEL=invalid \
...
test_macos_data_isolation() {
    ...
    printf '\n' | HOME="$test_home" PATH="$test_bin:$PATH" ZZZ_CHANNEL=stable \
        sh "$uninstall_script" >/dev/null
