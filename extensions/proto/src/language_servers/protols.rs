use zzz_extension_api::{
    self as zzz, Architecture, DownloadedFileType, GithubReleaseOptions, Os, Result,
...
impl ProtoLs {
    ...
    pub(crate) fn language_server_binary(
        ...
        let latest_release = zzz::latest_github_release(
            "coder3101/protols",
        ...
        let (os, arch) = zzz::current_platform();
        ...
        let release_suffix = match (os, arch) {
            ...
            _ => {
                return Err("Platform and architecture not supported by Protols".to_owned());
            }
        };
        ...
        zzz::download_file(&download_target.download_url, &version_dir, file_type)?;
