use std::fs;
use zzz::lsp::CompletionKind;
use zzz::{CodeLabel, CodeLabelSpan, LanguageServerId};
use zzz_extension_api::process::Command;
use zzz_extension_api::{self as zzz, Result};
...
impl TestExtension {
    fn language_server_binary_path(
        ...
        let command = match platform {
            zzz::Os::Linux | zzz::Os::Mac => Command::new("echo"),
            zzz::Os::Windows => Command::new("cmd").args(["/C", "echo"]),
        };
        ...
        let ext = "tar.gz";
        let download_type = zzz::DownloadedFileType::GzipTar;
        ...
        //
        // ext = "zip";
        // download_type = zzz::DownloadedFileType::Zip;
        ...
        let asset_name = format!(
            ...
            os = match platform {
                zzz::Os::Mac => "apple-darwin",
                zzz::Os::Linux => "unknown-linux-musl",
                zzz::Os::Windows => "pc-windows-msvc",
            },
        );
        ...
        if !fs::metadata(&binary_path).is_ok_and(|stat| stat.is_file()) {
            ...
            zzz::download_file(&asset.download_url, &version_dir, download_type)
                .map_err(|e| format!("failed to download file: {e}"))?;

            zzz::set_language_server_installation_status(
                language_server_id,
                &zzz::LanguageServerInstallationStatus::None,
            );
...
impl zzz::Extension for TestExtension {
    fn new() -> Self {
        Self {
    ...
    fn label_for_completion(
        ...
        completion: zzz::lsp::Completion,
    ) -> Option<zzz::CodeLabel> {
