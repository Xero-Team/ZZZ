...
impl PythonDebugAdapter {
    ...
    async fn request_args(
        ...
        if let Ok(console) = configuration.dot_get_mut("console") {
            // Use built-in ZZZ terminal if user did not explicitly provide a setting for console.
            if console.is_null() {
...
impl DebugAdapter for PythonDebugAdapter {
    ...
    async fn config_from_zzz_format(&self, zzz_scenario: ZZZDebugConfig) -> Result<DebugScenario> {
        let mut args = json!({
            "request": match zzz_scenario.request {
    ...
    async fn get_binary(
        ...
        let base_paths = ["cwd", "program", "module"]
            ...
            .chain(
                ...
                // (Which should always be the case because ZZZ defaults to the cwd worktree root)
