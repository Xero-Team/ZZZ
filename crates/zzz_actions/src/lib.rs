...
// If the zzz binary doesn't use anything in this crate, it will be optimized away
...
/// Opens a zzz:// URL within the application.
#[derive(Clone, PartialEq, Deserialize, JsonSchema, Action)]
#[action(namespace = zzz)]
#[serde(deny_unknown_fields)]
pub struct OpenZZZUrl {
    pub url: String,
}
...
#[action(namespace = zzz, no_json, no_register)]
...
actions!(
    ...
    [
        ...
        #[action(deprecated_aliases = ["zzz_actions::OpenSettingsEditor"])]
        OpenSettings,
        /// Opens the settings JSON file.
        #[action(deprecated_aliases = ["zzz_actions::OpenSettings"])]
        OpenSettingsFile,
        /// Opens project-specific settings.
        #[action(deprecated_aliases = ["zzz_actions::OpenProjectSettings"])]
        OpenProjectSettings,
        /// Opens the default keymap file.
        OpenDefaultKeymap,
        /// Opens the user keymap file.
        #[action(deprecated_aliases = ["zzz_actions::OpenKeymap"])]
        OpenKeymapFile,
        /// Opens the keymap editor.
        #[action(deprecated_aliases = ["zzz_actions::OpenKeymapEditor"])]
        OpenKeymap,
...
pub struct CreateWorktree {
    /// When this is None, ZZZ will randomly generate a worktree name.
...
pub struct Rerun {
    ...
    /// If it is not, environment variables such as ZZZ_COLUMN, ZZZ_FILE are gonna be the same as in the last execution of a task
