...
///
/// Has an absolute path, and may be set to be visible in ZZZ UI or not.
...
/// * a directory opened in ZZZ — may be added as a visible entry to the current worktree
///
...
impl BackgroundScanner {
    ...
    async fn process_events(&self, mut events: Vec<PathEvent>) {
        ...
        //
        // Certain directories may have FS changes, but do not lead to git data changes that ZZZ cares about.
        // Ignore these, to avoid ZZZ unnecessarily rescanning git metadata.
...

        {
            ...
            for (ix, event) in events.iter().enumerate() {
                ...
                // dropped. Once `fs` exposes per-volume case-sensitivity, fold this
