...
pub use wit::{
    ...
    zzz::extension::github::{
        GithubRelease, GithubReleaseAsset, GithubReleaseOptions, github_release_by_tag_name,
        latest_github_release,
    },
    zzz::extension::nodejs::{
        node_binary_path, npm_install_package, npm_package_installed_version,
        npm_package_latest_version,
    },
    zzz::extension::platform::{Architecture, Os, current_platform},
    zzz::extension::slash_command::{
...
pub mod lsp {
    pub use crate::wit::zzz::extension::lsp::{
        Completion, CompletionKind, InsertTextFormat, Symbol, SymbolKind,
    };
}

/// A result returned from a ZZZ extension.
...
pub trait Extension: Send + Sync {
    ...
    /// Locators are entities that convert a ZZZ task into a debug scenario.
    ///
    /// They can be provided even by extensions that don't provide a debug adapter.
    /// For all tasks applicable to a given buffer, ZZZ will query all locators to find one that can turn the task into a debug scenario.
    ...
    /// `dap_locator_create_scenario` if you're able to do that. Make sure to not fill out `build` field in that case, as that will prevent ZZZ from running second phase of resolution in such case.
...
/// Registers the provided type as a ZZZ extension.
///
...
macro_rules! register_extension {
    ($extension_type:ty) => {
        ...
        pub extern "C" fn __init_extension() {
            ...
            zzz_extension_api::register_extension(|| {
                Box::new(<$extension_type as zzz_extension_api::Extension>::new())
            });
        }
    };
}
...
#[unsafe(link_section = "zzz:api-version")]
#[doc(hidden)]
pub static ZZZ_API_VERSION: [u8; 6] = *include_bytes!(concat!(env!("OUT_DIR"), "/version_bytes"));
