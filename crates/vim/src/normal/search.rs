    ...
    fn replace_command(
        ...
        cx.spawn_in(window, async move |vim, cx| {
            ...
            search_bar.update_in(cx, |search_bar, window, cx| {
                ...
                drop(search_bar.search(&search_bar.query(cx), None, false, window, cx));
                vim.update(cx, |vim, cx| {
                    vim.move_cursor(
                        Motion::StartOfLine {
                            display_lines: false,
...
impl Replacement {
    // convert a vim query into something more usable by zzz.
...
mod test {
    ...
    async fn test_replace_gdefault(cx: &mut gpui::TestAppContext) {
        ...
        // Set the `gdefault` option in both ZZZ and Neovim.
