...
fn find_target(
    ...
    while let Some(ch) = chars.next() {
            ...
            && chars.peek().unwrap().is_ascii_hexdigit()
        {
            ...
            // (-> `0.82.46`) both increment the number under the cursor. When
            // the cursor is past the number (`111.ˇ.2`), `on_number` is false
            // and we still reset so the forward scan finds the number after the
            // dots.
            let on_number =
                ...
                && chars.peek().unwrap().is_digit(radix))
        {
            ...
            target.push(ch);
        } else if begin.is_some() && (is_num || is_toggle_word(&target)) {
            // End of matching
            end = Some(offset);
            break;
        } else if ch == '\n' {
