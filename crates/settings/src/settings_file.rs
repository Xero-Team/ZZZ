...
mod tests {
    ...
    async fn test_watch_config_file_reloads_when_parent_dir_is_symlink(cx: &mut TestAppContext) {
        cx.executor().allow_parking();
        let fs = FakeFs::new(cx.background_executor.clone());
        let config_settings_path = PathBuf::from("/root/.config/ZZZ/settings.json");
        let target_settings_path = PathBuf::from("/root/dotfiles/ZZZ/settings.json");
        ...
        fs.create_symlink(
            Path::new("/root/.config/ZZZ"),
            PathBuf::from("/root/dotfiles/ZZZ"),
        )
        .await
        .unwrap();
