...
pub enum OpenBehavior {
    ...
    /// Corresponds to `zzz -n`.
    AlwaysNew,
    /// Match broadly including subdirectories, and fall back to any existing
    /// window if no worktree matched. Corresponds to `zzz -a`.
    Add,
...
///
/// Note that in the main zzz binary, this variable is unset after it's read for the first time,
/// therefore it should always be accessed through the `FORCE_CLI_MODE` static.
pub const FORCE_CLI_MODE_ENV_VAR_NAME: &str = "ZZZ_FORCE_CLI_MODE";

/// Abstracts the transport for sending CLI responses (ZZZ → CLI).
///
