...
impl MoveToApplicationsRequest {
    ...
    async fn prompt(
        ...
        let prompt_title = cx.update(|_, cx| {
            tr(
                cx,
                "zzz.move_to_applications.prompt.title",
        ...
        let prompt_message = cx.update(|_, cx| {
            tr(
                cx,
                "zzz.move_to_applications.prompt.message",
                "ZZZ is running from a temporary location. Move it to Applications to finish installing it.",
            )
        })?;
        let yes_label = cx.update(|_, cx| tr(cx, "zzz.common.yes", "Yes"))?;
        let no_label = cx.update(|_, cx| tr(cx, "zzz.common.no", "No"))?;
        let dont_ask_again = cx.update(|_, cx| {
            tr(
                cx,
                "zzz.move_to_applications.prompt.dont_ask_again",
                "Don't ask me again",
            )
        })?;
        let response = cx
            ...
            0 => {
                ...
                if let Err(error) = move_to_applications(&self.app_path, cx).await {
                    let error_title = cx.update(|_, cx| {
                        tr(
                            cx,
                            "zzz.move_to_applications.error",
                            "Failed to move ZZZ to Applications",
                        )
                    })?;
                    let ok_label = cx.update(|_, cx| tr(cx, "zzz.common.ok", "Ok"))?;
                    workspace
                        .update_in(cx, |workspace, _window, cx| {
                            if let Some(modal) = workspace.active_modal::<InstallingZZZModal>(cx) {
...
pub struct InstallingZZZModal {
    focus_handle: FocusHandle,
    finished: bool,
}

impl InstallingZZZModal {
...
impl EventEmitter<DismissEvent> for InstallingZZZModal {}

impl ModalView for InstallingZZZModal {
...
impl Focusable for InstallingZZZModal {
    fn focus_handle(&self, _cx: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for InstallingZZZModal {
        ...
        v_flex()
            ...
            .child(
                div()
                    ...
                    .child(Label::new(tr(
                        cx,
                        "zzz.move_to_applications.installing",
                        "Installing ZZZ...",
                    ))),
            )
            .child(
                h_flex()
                    ...
                    .child(
                        v_flex()
                            ...
                            .child(Label::new(tr(
                                cx,
                                "zzz.move_to_applications.moving",
                                "Moving ZZZ to Applications",
                            )))
                            .child(
                                Label::new(tr(
                                    cx,
                                    "zzz.move_to_applications.reopen_when_complete",
