...
use edit_prediction::{EditPredictionModel, ZZZEditPredictionDelegate, fim};
use editor::Editor;
    ...
    match provider {
        ...
        EditPredictionProvider::Ollama | EditPredictionProvider::OpenAiCompatibleApi => {
            ...
            if matches!(format, EditPredictionPromptFormat::Zeta(_)) {
                Some(EditPredictionProviderConfig::ZZZ(EditPredictionModel::Zeta))
            } else {
                Some(EditPredictionProviderConfig::ZZZ(
...
fn assign_edit_prediction_provider(
    ...
    match provider_config {
        None => {
            editor.set_edit_prediction_provider::<ZZZEditPredictionDelegate>(None, window, cx);
        }
        ...
        Some(EditPredictionProviderConfig::ZZZ(model)) => {
            let ep_store = edit_prediction::EditPredictionStore::global(client, &user_store, cx);
            ...
            if let Some(project) = editor.project() {
                ...
                let provider = cx.new(|cx| {
                    ZZZEditPredictionDelegate::new(project.clone(), &client, &user_store, cx)
                });
                editor.set_edit_prediction_provider(Some(provider), window, cx);
            }
        }
