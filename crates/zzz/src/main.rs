            ...
            if let Ok(window) = cx.open_window(gpui::WindowOptions::default(), |_, cx| {
                ...
                window
                    .update(cx, |_, window, cx| {
                        let title =
                            app_i18n::tr(cx, "zzz.launch.failed_title", "ZZZ failed to launch");
                        let exit = app_i18n::tr(cx, "zzz.launch.exit", "Exit");
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    let _launch_failed = app_i18n::tr(_cx, "zzz.launch.failed_title", "ZZZ failed to launch");
    eprintln!(
        "{}: {e:?}. See the local troubleshooting documentation for help.",
        app_i18n::tr(
            _cx,
            "zzz.launch.failed_to_open_window",
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.zzz.Oops";
            proxy
                .add_notification(
                    notification_id,
...
fn main() {
    ...
    if args.record_etw_trace {
        ...
        if let Err(error) = etw_tracing::record_etw_trace(args.etw_zzz_pid, &etw_socket) {
    ...
    // `zzz --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    #[cfg(target_os = "windows")]
    match util::get_zzz_cli_path() {
        Ok(path) => askpass::set_askpass_program(path),
        Err(err) => {
            eprintln!("Error: {}", err);
            if std::option_env!("ZZZ_BUNDLE").is_some() {
                process::exit(1);
            }
        }
    }
    ...
    if args.system_specs {
        ...
        println!("ZZZ System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    let failed_single_instance_check = if *zzz_env_vars::ZZZ_STATELESS
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::zzz::listen_for_cli_connections(open_listener.clone()).is_err()
        }

        #[cfg(target_os = "windows")]
        {
            !crate::zzz::windows_only_instance::handle_single_instance(open_listener.clone(), &args)
        }

        #[cfg(target_os = "macos")]
        {
            use zzz::mac_only_instance::*;
            ensure_only_instance() != IsOnlyInstance::Yes
        }
    };
    if failed_single_instance_check {
        println!("zzz is already running");
        return;
    }
    ...
    app.run(move |cx| {
        ...
        zzz::init(cx);
        #[cfg(target_os = "macos")]
        zzz::move_to_applications::init(cx);
            ...
            .and_then(|request| OpenRequest::parse(request, cx).log_err())
        {
            Some(request) if request.is_focus_app_only() => cx.spawn({
                ...
                async move |cx| {
                    if let Err(error) = restore_or_create_workspace(app_state, cx).await {
...
fn handle_open_request(request: OpenRequest, app_state: Arc<AppState>, cx: &mut App) {
    if let Some(kind) = request.kind {
        match kind {
                    ...
                    multi_workspace.update(cx, |multi_workspace, window, cx| {
                        multi_workspace.workspace().update(cx, |workspace, cx| {
                            if let Some(panel) = workspace.focus_panel::<AgentPanel>(window, cx) {
                                ...
                                log::warn!(
                                    "zzz://agent received but AgentPanel is not registered (is `disable_ai` enabled?)"
                                );
                            }
                        });
                    })
            ...
            OpenRequestKind::InstallSkill { content } => {
                cx.spawn(async move |cx| {
                    ...
                    multi_workspace.update(cx, |_multi_workspace, window, cx| {
                        ...
                        window.dispatch_action(
                            Box::new(zzz_actions::assistant::OpenRulesLibrary::default()),
                            cx,
                        );
                    })
                })
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("zzz://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                // zzz://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // zzz://settings/languages/Rust/tab_size  - SUPPORT
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| match setting_path {
                        None => window.dispatch_action(Box::new(zzz_actions::OpenSettings), cx),
                        Some(setting_path) => window.dispatch_action(
                            Box::new(zzz_actions::OpenSettingsAt { path: setting_path }),
                            cx,
                        ),
                    })
                })
            ...
            OpenRequestKind::GitCommit { sha } => {
                let base_open_options = zzz::open_options_for_request(
    ...
    if let Some(connection_options) = request.remote_connection {
        ...
        let base_open_options = zzz::open_options_for_request(open_behavior, &location, cx);
        cx.spawn(async move |cx| {
        ...
        if error_count > 0 {
            let message = cx.update(|cx| {
                if error_count == 1 {
                    app_i18n::tr(
                        cx,
                        "zzz.restore_workspace.failed_single",
                        "Failed to restore 1 workspace. Check logs for details.",
                    )
                } else {
                    app_i18n::tr(
                        cx,
                        "zzz.restore_workspace.failed_multiple",
...
#[derive(Parser, Debug)]
#[command(name = "zzz", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    /// On macOS, the default is `~/Library/Application Support/ZZZ`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/zzz`.
    /// On Windows, the default is `%LOCALAPPDATA%\ZZZ`.
    ...
    /// Instructs zzz to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents ZZZ from starting, so you can't run `zzz: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Run zzz in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
    ...
    /// The PID of the ZZZ process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
    etw_zzz_pid: Option<u32>,

    /// Unix socket path for IPC with the parent ZZZ process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
