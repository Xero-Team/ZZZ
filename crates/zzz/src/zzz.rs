...
pub fn init(cx: &mut App) {
    ...
    // When ZZZ logs to stdout rather than the log file, avoid registering
    ...
    cx.on_action(|_: &zzz_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
                ...
                tr(
                    cx,
                    "zzz.bundled_file.open_source_license_attribution",
    ...
    .on_action(|&zzz_actions::OpenKeymapFile, cx| {
    ...
    .on_action(|_: &ShowDefaultSemanticTokenRules, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                ...
                tr(
                    cx,
                    "zzz.bundled_file.default_semantic_token_rules",
    ...
    .on_action(|_: &OpenDefaultSettings, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                ...
                tr(cx, "zzz.bundled_file.default_settings", "Default Settings").into(),
                "JSON",
                window,
                cx,
            );
    ...
    .on_action(|_: &zzz_actions::OpenDefaultKeymap, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
                ...
                tr(
                    cx,
                    "zzz.bundled_file.default_key_bindings",
...
pub fn build_window_options(display_uuid: Option<Uuid>, cx: &mut App) -> WindowOptions {
    ...
    WindowOptions {
        ...
        is_movable: true,
        // ZZZ draws its own titlebar and moves the window via [`Window::start_window_move`],
        ...
        cx.subscribe_in(&workspace_handle, window, {
            move |workspace, _, event, window, cx| match event {
                ...
                workspace::Event::OpenBundledFile {
                    ...
                    open_bundled_file(workspace, text.clone(), title.clone(), language, window, cx)
                }
                _ => {}
            }
        })
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = tr(
            cx,
            "zzz.file_watcher.inotify_error_detail",
        ...
        let prompt = window.prompt(
            ...
            &tr(
                cx,
                "zzz.file_watcher.inotify_error",
                "Could not start inotify",
            ),
            Some(&message),
            &[tr(
                cx,
                "zzz.file_watcher.troubleshoot_and_quit",
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = tr(
            cx,
            "zzz.file_watcher.read_directory_changes_error_detail",
        ...
        let prompt = window.prompt(
            ...
            &tr(
                cx,
                "zzz.file_watcher.read_directory_changes_error",
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("ZZZ_ALLOW_EMULATED_GPU").is_err() {
        let (graphics_api, docs_url) = if cfg!(target_os = "windows") {
            (
                tr(cx, "zzz.gpu.graphics_api.directx", "DirectX"),
                "local Windows troubleshooting documentation",
            )
        } else {
            (
                tr(cx, "zzz.gpu.graphics_api.vulkan", "Vulkan"),
                "local Linux troubleshooting documentation",
            )
        };
        let message = tr(
            cx,
            "zzz.gpu.unsupported_detail",
            "ZZZ uses {} for rendering and requires a compatible GPU.\n\nCurrently you are using a software emulated GPU ({}) which\nwill result in awful performance.\n\nFor troubleshooting see: {}\nSet ZZZ_ALLOW_EMULATED_GPU=1 env var to permanently override.",
        )
        .replacen("{}", &graphics_api, 1)
        .replacen("{}", &specs.device_name, 1)
        .replacen("{}", docs_url, 1);
        let prompt = window.prompt(
            PromptLevel::Critical,
            &tr(cx, "zzz.gpu.unsupported", "Unsupported GPU"),
            Some(&message),
            &[
                tr(cx, "zzz.gpu.skip", "Skip").as_str(),
                tr(
                    cx,
                    "zzz.file_watcher.troubleshoot_and_quit",
                    "Troubleshoot and Quit",
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|_, action: &OpenZZZUrl, _, cx| {
        ...
        .register_action(|workspace, action: &zzz_actions::OpenRemote, window, cx| {
        ...
        .register_action({
            ...
            move |_, action: &zzz_actions::IncreaseUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zzz_actions::DecreaseUiFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zzz_actions::ResetUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zzz_actions::IncreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zzz_actions::DecreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zzz_actions::ResetBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &zzz_actions::ResetAllZoom, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
                        settings.theme.ui_font_size = None;
                        settings.theme.buffer_font_size = None;
        ...
        .register_action(|_, _: &install_cli::RegisterZZZScheme, window, cx| {
            cx.spawn_in(window, async move |workspace, cx| {
                install_cli::register_zzz_scheme(cx).await?;
                workspace.update_in(cx, |workspace, _, cx| {
                    struct RegisterZZZScheme;
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &zzz_actions::project_panel::ToggleFocus,
             window: &mut Window,
             cx: &mut Context<Workspace>| {
...
fn open_about_window(cx: &mut App) {
                    ...
                    .child(
                        v_flex()
                            ...
                            .when_some(self.commit.clone(), |this, commit| {
                                this.child(
                                    Label::new(tr(cx, "zzz.about.commit", "Commit"))
                                        .color(Color::Muted)
                            ...
                            .child(
                                Label::new(tr(cx, "zzz.about.version", "Version"))
                                    .color(Color::Muted)
                                    .size(LabelSize::XSmall),
                            )
                        ...
                        h_flex()
                            ...
                            .child(
                                div()
                                    ...
                                    .child(
                                        Button::new("ok", tr(cx, "zzz.common.ok", "Ok"))
                                            .full_width()
                            ...
                            .child(
                                div()
                                    ...
                                    .child(
                                        Button::new("copy", tr(cx, "zzz.common.copy", "Copy"))
                                            .full_width()
    ...
    let about_title = tr(cx, "menu.about", "About ZZZ");

    cx.open_window(
        WindowOptions {
            titlebar: Some(TitlebarOptions {
                title: Some(about_title.into()),
                appears_transparent: true,
            ...
            let answer = multi_workspace
                .update(cx, |_, window, cx| {
                    window.prompt(
                        ...
                        &tr(
                            cx,
                            "zzz.quit.confirm_title",
                            "Are you sure you want to quit?",
                        ),
                        None,
                        &[
                            tr(cx, "zzz.quit.quit", "Quit").as_str(),
                            tr(cx, "zzz.quit.cancel", "Cancel").as_str(),
                        ],
                        cx,
                    )
                })
...
fn open_log_file(workspace: &mut Workspace, window: &mut Window, cx: &mut Context<Workspace>) {
    ...
    let log_title = tr(cx, "zzz.log.title", "Log");
    let log_breadcrumb = tr(cx, "zzz.log.last_lines_in", "Last {} lines in {}")
        .replacen("{}", &MAX_LINES.to_string(), 1)
        .replacen("{}", &paths::log_file().display().to_string(), 1);
        ...
        maybe!(async move {
            ...
            let editor = cx
                .new_window_entity(|window, cx| {
                    ...
                    editor.set_breadcrumb_header(log_breadcrumb);
...
fn notify_settings_errors(result: settings::SettingsParseResult, is_user: bool, cx: &mut App) {
    let settings_scope = tr(
        cx,
        if is_user {
            "zzz.settings.scope.user"
        } else {
            "zzz.settings.scope.global"
        },
        if is_user { "user" } else { "global" },
    );

    if let settings::ParseStatus::Failed { error: err } = &result.parse_status {
        log::error!("Failed to load {} settings: {err}", settings_scope);
    }
        ...
        settings::MigrationStatus::Failed { error: err } => {
            if !showed_parse_error {
                show_app_notification(id, cx, move |cx| {
                    let message = tr(
                        cx,
                        "zzz.settings.failed_to_migrate",
...
fn show_keymap_file_json_error(
    ...
    let message: SharedString = tr(
        cx,
        "zzz.keymap.invalid_keymap_file",
        "JSON parse error in keymap file. Bindings not reloaded.\n\n{}",
    )
    .replacen("{}", &error.to_string(), 1)
    .into();
    show_app_notification(notification_id, cx, move |cx| {
        let open_keymap_file = tr(cx, "menu.settings.open_keymap_file", "Open Keymap File");
        cx.new(|cx| {
            MessageNotification::new(message.clone(), cx)
                .primary_message(open_keymap_file)
                .primary_icon(IconName::Settings)
                .primary_on_click(|window, cx| {
...
fn show_keymap_file_load_error(
    ...
    show_markdown_app_notification(
        notification_id,
        error_message,
        tr(cx, "menu.settings.open_keymap_file", "Open Keymap File").into(),
        |window, cx| {
            window.dispatch_action(zzz_actions::OpenKeymapFile.boxed_clone(), cx);
            cx.emit(DismissEvent);
        },
...
fn open_project_debug_tasks_file(
    workspace: &mut Workspace,
    _: &zzz_actions::OpenProjectDebugTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    open_local_file(
...
fn open_bundled_file(
    ...
    let existing = workspace.items_of_type::<Editor>(cx).find(|editor| {
        editor.read_with(cx, |editor, cx| {
            editor.read_only(cx)
                && editor.title(cx).as_ref() == title.as_ref()
                && editor
                    .buffer()
                    .read(cx)
                    .as_singleton()
        ...
        workspace
            .update_in(cx, move |workspace, window, cx| {
                ...
                cx.spawn_in(window, async move |workspace, cx| {
                    ...
                    let title_for_editor = title.clone();
                    let buffer = cx.new(|cx| {
                        MultiBuffer::singleton(buffer, cx).with_title(title_for_buffer.to_string())
                    });
                    workspace.update_in(cx, |workspace, window, cx| {
                        workspace.add_item_to_active_pane(
                            Box::new(cx.new(|cx| {
                                ...
                                editor.set_breadcrumb_header(title_for_editor.to_string());
                                editor
                            })),
                            None,
                            true,
