...
impl VsCodeDebugTaskDefinition {
    fn try_to_zzz(mut self, replacer: &EnvVariableReplacer) -> anyhow::Result<DebugScenario> {
...
impl TryFrom<VsCodeDebugTaskFile> for DebugTaskFile {
    ...
    fn try_from(file: VsCodeDebugTaskFile) -> Result<Self, Self::Error> {
        ...
        let templates = file
            .configurations
            .into_iter()
            .filter_map(|config| config.try_to_zzz(&replacer).log_err())
            .collect::<Vec<_>>();
...
mod tests {
    ...
    fn test_parsing_vscode_launch_json() {
        ...
        let zzz = DebugTaskFile::try_from(parsed).expect("converting to ZZZ debug templates");
        pretty_assertions::assert_eq!(
            ...
            DebugTaskFile(vec![DebugScenario {
                ...
                config: json!({
                    "request": "launch",
                    "program": "${ZZZ_WORKTREE_ROOT}/xyz.js",
                    "showDevDebugOutput": false,
                    "stopOnEntry": true,
                    "args": [
                        "--foo",
                        "${ZZZ_WORKTREE_ROOT}/thing",
                    ],
                    "cwd": "${ZZZ_WORKTREE_ROOT}/${FOO}/sub",
                    "env": {
                        "X": "Y",
                    },
                    "type": "node",
