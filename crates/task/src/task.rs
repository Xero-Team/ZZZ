//! Baseline interface of Tasks in ZZZ: all tasks in ZZZ are intended to use those for implementing their own logic.
...
pub use debug_format::{
    ...
    Request, TcpArgumentsTemplate, ZZZDebugConfig,
};
...
pub use zzz_actions::RevealTarget;
...
/// Contains all information needed by ZZZ to spawn a new terminal tab for the given task.
...
/// Variables, available for use in [`TaskContext`] when a ZZZ's [`TaskTemplate`] gets resolved into a [`ResolvedTask`].
...
impl FromStr for VariableName {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let without_prefix = s.strip_prefix(ZZZ_VARIABLE_NAME_PREFIX).ok_or(())?;
        let value = match without_prefix {
            ...
            _ => {
                if let Some(custom_name) =
                    without_prefix.strip_prefix(ZZZ_CUSTOM_VARIABLE_NAME_PREFIX)
                {
...
pub const ZZZ_VARIABLE_NAME_PREFIX: &str = "ZZZ_";
const ZZZ_CUSTOM_VARIABLE_NAME_PREFIX: &str = "CUSTOM_";

impl std::fmt::Display for VariableName {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::File => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}FILE"),
            Self::Filename => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}FILENAME"),
            Self::RelativeFile => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}RELATIVE_FILE"),
            Self::RelativeDir => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}RELATIVE_DIR"),
            Self::Dirname => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}DIRNAME"),
            Self::Stem => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}STEM"),
            Self::WorktreeRoot => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}WORKTREE_ROOT"),
            Self::Symbol => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}SYMBOL"),
            Self::Row => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}ROW"),
            Self::Column => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}COLUMN"),
            Self::SelectedText => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}SELECTED_TEXT"),
            Self::Language => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}LANGUAGE"),
            Self::RunnableSymbol => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}RUNNABLE_SYMBOL"),
            Self::PickProcessId => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}PICK_PID"),
            Self::MainGitWorktree => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}MAIN_GIT_WORKTREE"),
            Self::GitSha => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}GIT_SHA"),
            Self::GitShaShort => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}GIT_SHA_SHORT"),
            Self::GitRepositoryName => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}GIT_REPOSITORY_NAME"),
            Self::GitRepositoryPath => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}GIT_REPOSITORY_PATH"),
            Self::GitRef => write!(f, "{ZZZ_VARIABLE_NAME_PREFIX}GIT_REF"),
            Self::Custom(s) => write!(
                f,
                "{ZZZ_VARIABLE_NAME_PREFIX}{ZZZ_CUSTOM_VARIABLE_NAME_PREFIX}{s}"
            ),
        }
    }
}

/// Container for predefined environment variables that describe state of ZZZ at the time the task was spawned.
...
/// Keeps all ZZZ-related state inside, used to produce a resolved task out of its template.
...
pub struct TaskContext {
    ...
    /// Environment variables obtained when loading the project into ZZZ.
...
type ZZZEnvVariable = String;

struct EnvVariableReplacer {
    variables: HashMap<VsCodeEnvVariable, ZZZEnvVariable>,
    commands: HashMap<VsCodeCommand, ZZZEnvVariable>,
}

impl EnvVariableReplacer {
    fn new(variables: HashMap<VsCodeEnvVariable, ZZZEnvVariable>) -> Self {
        Self {
            variables,
    ...
    fn with_commands(
        mut self,
        commands: impl IntoIterator<Item = (VsCodeCommand, ZZZEnvVariable)>,
    ) -> Self {
    ...
    // Replaces occurrences of VsCode-specific environment variables with ZZZ equivalents.
    fn replace(&self, input: &str) -> String {
        shellexpand::env_with_context_no_errors(&input, |var: &str| {
            ...
            if let Some(substitution) = self.variables.get(variable_name) {
                // Got a VSCode->ZZZ hit, perform a substitution
