...
pub struct AllLanguageSettingsContent {
    ...
    /// with languages.
    pub file_types: Option<HashMap<Arc<str>, ExtendingSet<String>>>,
}
...
pub struct LanguageSettingsContent {
    ...
    /// ZZZ's Prettier integration settings.
    ...
    /// when you type '(', ZZZ will automatically add a closing ')' at the correct position.
    ///
    /// Default: true
    pub use_autoclose: Option<bool>,
    /// Whether to automatically surround text with characters for you. For example,
    /// when you select text and type '(', ZZZ will automatically surround text with ().
    ///
    /// Default: true
...
#[serde(rename_all = "snake_case")]
pub enum Formatter {
    /// Format files using ZZZ's Prettier integration (if applicable),
    /// or falling back to formatting via language server.
    #[default]
    Auto,
    /// Do not format code.
    None,
    /// Format code using ZZZ's Prettier integration.
    Prettier,
...
pub struct LanguageTaskSettingsContent {
    ...
    pub enabled: Option<bool>,
    /// Use LSP tasks over ZZZ language extension ones.
    /// If no LSP tasks are returned due to error/timeout or regular execution,
    /// ZZZ language extension tasks will be used instead.
    ///
    /// Other ZZZ tasks will still be shown:
    /// * ZZZ task from either of the task config file
    /// * ZZZ task from history (e.g. one-off task was spawned before)
