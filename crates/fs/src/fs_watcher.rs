...
///
/// Can be overridden with the `ZZZ_FILE_WATCHER_MODE` environment variable:
...
pub fn requires_poll_watcher(path: &Path) -> bool {
    match std::env::var("ZZZ_FILE_WATCHER_MODE")
        .as_deref()
        .unwrap_or("auto")
    {
        "native" => return false,
...
fn case_insensitive_path(_path: &Path) -> bool {
    // Upstream followed up the initial Linux statx probe with a temporary
    // rollback because it broke CI builds. Keep the watcher logic, but fall
    // back to "case-sensitive" on Linux until a portable casefold check lands.
    false
}
...
impl GlobalWatcher {
    ...
    fn ensure_native_watcher(&self) -> anyhow::Result<()> {
        ...
        // CORE excludes Access events, which ZZZ discards anyway. Without this,
...
static POLL_INTERVAL: LazyLock<Duration> = LazyLock::new(|| {
    let poll_ms: u64 = std::env::var("ZZZ_FILE_WATCHER_POLL_MS")
        .ok()
...
static NATIVE_WATCH_LIMIT_COOLDOWN: LazyLock<Duration> = LazyLock::new(|| {
    let cooldown_seconds: u64 = std::env::var("ZZZ_NATIVE_WATCH_LIMIT_COOLDOWN_SECONDS")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(5)
        .clamp(0, 300);
