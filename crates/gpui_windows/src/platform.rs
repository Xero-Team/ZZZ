...
impl Platform for WindowsPlatform {
    ...
    fn restart(&self, binary_path: Option<PathBuf>, arguments: Vec<OsString>) {
        ...
        let script = r#"
            $pidToWaitFor = $env:ZZZ_RESTART_PID
            $exePath = $env:ZZZ_RESTART_EXECUTABLE
            $argumentList = $env:ZZZ_RESTART_ARGUMENTS

            [Environment]::SetEnvironmentVariable("ZZZ_RESTART_PID", $null)
            [Environment]::SetEnvironmentVariable("ZZZ_RESTART_EXECUTABLE", $null)
            [Environment]::SetEnvironmentVariable("ZZZ_RESTART_ARGUMENTS", $null)
        ...
        self.foreground_executor
            .spawn(async move {
                ...
                command
                    .arg("-command")
                    .arg(script)
                    .env("ZZZ_RESTART_PID", pid.to_string())
                    .env("ZZZ_RESTART_EXECUTABLE", app_path)
                    .env("ZZZ_RESTART_ARGUMENTS", arguments);
                #[allow(
...
const PLATFORM_WINDOW_CLASS_NAME: PCWSTR = w!("ZZZ::PlatformWindow");
