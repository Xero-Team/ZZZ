...
impl DirectWriteState {
    ...
    fn rasterize_color(
        ...
        // (`Map`/`Unmap`/`Draw`/`CopyResource`), which `DirectXRenderer` and
        // `DirectXAtlas` also touch. An immediate `ID3D11DeviceContext` is not
        // thread-safe, so this must only run on the main UI thread. Text
        // rasterization never leaves that thread.
        ...
        // Release the mapping after copying out the rows. Leaving the staging
        // texture mapped pins the resource and leaks the mapping.
...
/// nonzero length returns `null_error_message` as `E_INVALIDARG`.
///
/// # Safety
///
