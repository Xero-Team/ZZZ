...
impl ReqwestClient {
    ...
    pub fn new() -> Self {
        Self::builder(Some(Duration::from_secs(30)))
            .build()
    ...
    pub fn user_agent(agent: &str) -> anyhow::Result<Self> {
        ...
        let client = Self::builder(Some(Duration::from_secs(30)))
            .default_headers(map)
            .build()?;
        Ok(client.into())
    }
    ...
    pub fn proxy_and_user_agent(proxy: Option<Url>, user_agent: &str) -> anyhow::Result<Self> {
        Self::proxy_user_agent_and_read_timeout(proxy, user_agent, Some(Duration::from_secs(30)))
    }
