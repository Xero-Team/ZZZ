...
///
/// `AllowThread` and `AllowAlways` both use `PermissionOptionKind::AllowAlways`;
/// the id distinguishes their persistence scope.
...
pub fn sandbox_authorization_details_from_meta(
    ...
    meta.as_ref()
        .and_then(|meta| meta.get(SANDBOX_AUTHORIZATION_META_KEY))
...
fn text_resource_render_mode(mime_type: Option<&str>) -> TextResourceRenderMode {
    ...
    if matches!(mime_type.as_str(), "text/markdown" | "text/x-markdown") {
        return TextResourceRenderMode::Markdown;
    }

    let language = match mime_type.as_str() {
        "application/json" | "text/json" | "text/x-json" => Some("json"),
        ...
        "text/x-shellscript" | "application/x-shellscript" | "application/x-sh" => Some("sh"),
        ...
        "text/x-c" => Some("c"),
        "text/x-c++" => Some("cpp"),
        "application/graphql" => Some("graphql"),
        "application/toml" => Some("toml"),
        "application/yaml" | "application/x-yaml" => Some("yaml"),
        "text/plain" => None,
        other if other.ends_with("+json") => Some("json"),
        other if other.ends_with("+xml") => Some("xml"),
        other if other.ends_with("+yaml") || other.ends_with("+yml") => Some("yaml"),
        other if other.starts_with("text/") => None,
...
mod tests {
    ...
    #[test]
    fn sandbox_authorization_details_round_trip_through_meta() {
        let details = SandboxAuthorizationDetails {
            network: true,
            allow_fs_write_all: false,
            unsandboxed: false,
            write_paths: vec![PathBuf::from("/tmp/build"), PathBuf::from("/tmp/cache")],
        };

        let meta = Some(meta_with_sandbox_authorization(details.clone()));

        assert_eq!(
            sandbox_authorization_details_from_meta(&meta),
            Some(details)
        );
    }

    #[test]
    fn sandbox_permission_ids_round_trip() {
        for permission in [
            SandboxPermission::AllowOnce,
            SandboxPermission::AllowThread,
            SandboxPermission::AllowAlways,
            SandboxPermission::Deny,
        ] {
            assert_eq!(
                SandboxPermission::from_id(permission.as_id()),
                Some(permission)
            );
        }
    }
    ...
    async fn test_terminal_exit_preserves_visible_scrollback(cx: &mut gpui::TestAppContext) {
        ...
        let content = thread.read_with(cx, |thread, cx| {
            let term = thread.terminal(terminal_id).unwrap();
    ...
    async fn test_thinking_concatenation(cx: &mut gpui::TestAppContext) {
        ...
        thread
            .update(cx, |thread, cx| thread.send_raw("Hello from ZZZ!", cx))
            .await
            .unwrap();
    ...
    async fn test_ignore_echoed_user_message_chunks_during_active_turn(
        ...
        let output = thread.read_with(cx, |thread, cx| thread.to_markdown(cx));
        assert_eq!(output.matches("Hello from ZZZ!").count(), 1);
    }
    ...
    /// loading state until ZZZ restarts.
    #[gpui::test]
