...
impl MentionUri {
    ...
    ///
    /// Mention URIs stay strict so they can round-trip, while hyperlinks may
    /// contain a Windows shell path rather than a URI.
    ...
    /// Returns the literal interpretation of a bare hyperlink path when
    /// decoding percent escapes would produce a different path.
...
/// Returns a bare absolute path target, with agent-added backticks removed.
...
/// Decodes percent escapes without decoding separators, so an escaped
/// separator cannot change which directory is opened.
fn decode_path_escapes(input: &str) -> Cow<'_, str> {
    ...
    String::from_utf8(decoded).map_or_else(|_| Cow::Borrowed(input), Cow::Owned)
}

/// Converts common Windows path spellings in Markdown to the native spelling
/// used for worktree-path comparison.
...
mod tests {
    ...
    #[test]
    fn test_parse_windows_file_uri_with_spaces() {
        let parsed =
    ...
    #[test]
    fn test_parse_windows_hyperlink_paths() {
        let selection = MentionUri::parse_hyperlink(
            "/C:/Projects/Example%20Workspace/Cargo.toml:2",
            PathStyle::Windows,
        )
        .unwrap();
        assert_eq!(
            selection,
            MentionUri::Selection {
                abs_path: Some(PathBuf::from("C:\\Projects\\Example Workspace\\Cargo.toml")),
                line_range: 1..=1,
                column: None,
            }
        );

        let file = MentionUri::parse_hyperlink(
        ...
        assert_eq!(
            file,
            MentionUri::File {
                abs_path: PathBuf::from("C:\\Projects\\Example Workspace\\AGENTS.md")
            }
        );
    }

    #[test]
    fn test_hyperlink_literal_path_preserves_escaped_separator() {
        let parsed =
            MentionUri::parse_hyperlink("/project/a%20b%2Fc.rs", PathStyle::Posix).unwrap();
        assert_eq!(
            parsed,
            MentionUri::File {
                abs_path: PathBuf::from("/project/a b%2Fc.rs")
            }
        );
        assert_eq!(
            MentionUri::parse_hyperlink_literal("/project/a%20b.rs", PathStyle::Posix),
            Some(MentionUri::File {
                abs_path: PathBuf::from("/project/a%20b.rs")
            })
        );
    }
    ...
    fn test_parse_untitled_selection_uri() {
        let selection_uri = uri!("zzz:///agent/untitled-buffer#L1:10");
        let parsed = MentionUri::parse(selection_uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Selection {
                abs_path: None,
    ...
    fn test_parse_thread_uri() {
        let thread_uri = "zzz:///agent/thread/session123?name=Thread+name";
    ...
    fn test_parse_rule_uri() {
        let rule_uri = "zzz:///agent/rule/d8694ff2-90d5-4b6f-be33-33c1763acd52?name=Some+rule";
    ...
    fn test_parse_diagnostics_uri() {
        let uri = "zzz:///agent/diagnostics?include_warnings=true";
    ...
    fn test_parse_diagnostics_uri_warnings_only() {
        let uri = "zzz:///agent/diagnostics?include_warnings=true&include_errors=false";
        let parsed = MentionUri::parse(uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Diagnostics {
                include_errors,
    ...
    #[test]
    fn test_invalid_zzz_path() {
        assert!(MentionUri::parse("zzz:///invalid/path", PathStyle::local()).is_err());
        assert!(MentionUri::parse("zzz:///agent/unknown/test", PathStyle::local()).is_err());
    }
    ...
    fn test_parse_absolute_windows_path() {
        let file_path = "C:\\Users\\zzz\\project\\main.rs";
        let parsed = MentionUri::parse(file_path, PathStyle::Windows).unwrap();
        match &parsed {
            MentionUri::File { abs_path } => {
                assert_eq!(abs_path, Path::new("C:\\Users\\zzz\\project\\main.rs"));
            }
            _ => panic!("Expected File variant"),
        }
    }
    ...
    fn test_parse_absolute_windows_file_path_with_row() {
        let file_path = "C:\\Users\\zzz\\project\\main.rs:42";
        let parsed = MentionUri::parse(file_path, PathStyle::Windows).unwrap();
        match &parsed {
            MentionUri::Selection {
                abs_path: path,
                ...
                assert_eq!(
                    path.as_ref().unwrap(),
                    Path::new("C:\\Users\\zzz\\project\\main.rs")
                );
                assert_eq!(line_range.start(), &41);
                assert_eq!(line_range.end(), &41);
            }
    ...
    fn test_parse_absolute_windows_file_path_with_fragment_line() {
        let file_path = "C:\\Users\\zzz\\project\\main.rs#L42";
        let parsed = MentionUri::parse(file_path, PathStyle::Windows).unwrap();
        match &parsed {
            MentionUri::Selection {
                abs_path: path,
    ...
    fn test_parse_terminal_selection_uri() {
        let terminal_uri = "zzz:///agent/terminal-selection?lines=42";
        ...
        // Test single line
        let single_line_uri = "zzz:///agent/terminal-selection?lines=1";
