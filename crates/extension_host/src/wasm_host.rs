...
impl extension::Extension for WasmExtension {
    ...
    async fn dap_config_to_scenario(&self, config: ZZZDebugConfig) -> Result<DebugScenario> {
        self.call(|extension, store| {
            async move {
                let kind = extension
    ...
    pub fn load_extension(
        ...
        let compile_task = {
            ...
            executor.spawn(async move {
                let zzz_api_version = parse_wasm_extension_version(&manifest_id, &wasm_bytes)?;
                let component = Component::from_binary(&engine, &wasm_bytes)
                    .map_err(|error| anyhow!("failed to compile wasm component: {error}"))?;

                anyhow::Ok((zzz_api_version, component))
            })
        };

        let load_extension = |zzz_api_version: Version, component| async move {
        ...
        cx.spawn(async move |cx| {
            let (zzz_api_version, component) = compile_task.await?;
