...
impl WgpuContext {
    ...
    fn new_with_options(
        ...
        let device_id_filter = match std::env::var("ZZZ_DEVICE_ID") {
            Ok(val) => parse_pci_id(&val)
                .context("Failed to parse device ID from `ZZZ_DEVICE_ID` environment variable")
                .log_err(),
            Err(std::env::VarError::NotPresent) => None,
            err => {
                err.context("Failed to read value of `ZZZ_DEVICE_ID` environment variable")
                    .log_err();
                None
            }
        };
    ...
    async fn select_adapter_and_device(
        ...
        if let Some(device_id) = device_id_filter {
            log::info!("ZZZ_DEVICE_ID filter: {:#06x}", device_id);
        }

        // Sort adapters into a single priority order. Tiers (from highest to lowest):
        //
        // 1. ZZZ_DEVICE_ID match — explicit user override
