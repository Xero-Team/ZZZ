...
pub fn guess_compositor() -> &'static str {
    if std::env::var_os("ZZZ_HEADLESS").is_some() {
        return "Headless";
    }
...
impl WindowButtonLayout {
    /// Returns ZZZ's built-in fallback button layout for Linux titlebars.
...
/// ZZZ's interface for handling text input from the platform's IME system
