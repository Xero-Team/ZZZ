...
/// variables to ZZZ, matching LanguageSettings fields.
...
pub struct ModelineSettings {
    ...
    /// Emacs modeline variables that were parsed but not mapped to ZZZ settings.
    /// Stored as (variable-name, value) pairs.
    pub emacs_extra_variables: Vec<(String, String)>,
    /// Vim modeline options that were parsed but not mapped to ZZZ settings.
