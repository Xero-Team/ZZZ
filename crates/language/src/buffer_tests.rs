...
fn test_select_language(cx: &mut App) {
    ...
    assert_eq!(
        registry
            .language_for_file(&file("zzz/cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("zzz/a.cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("zzz/sumk"), None, cx)
            .map(|l| l.name()),
        None
    );
}
...
async fn test_markdown_inline_html_highlighting(cx: &mut TestAppContext) {
    ...
    let markdown_inline_language = Arc::new(
        ...
        .expect("Markdown inline highlights query should parse")
        .with_injection_query(include_str!(
            "../../grammars/src/markdown-inline/injections.scm"
        ))
        .expect("Markdown inline injections query should parse"),
    );
    let html_language = Arc::new(
        ...
        .expect("HTML highlights query should parse"),
    );
...
fn test_random_collaboration(cx: &mut App, mut rng: StdRng) {
    ...
    let network = Arc::new(Mutex::new(Network::new(StdRng::seed_from_u64(
        rng.random(),
    ))));
