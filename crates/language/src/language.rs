//! The `language` crate provides a large chunk of ZZZ's language-related
...
pub trait LspAdapter: 'static + Send + Sync + DynLspInstaller {
    ...
    /// provide autocompletion and diagnostics in ZZZ setting and keybind
    /// files
    fn is_primary_zzz_json_schema_adapter(&self) -> bool {
        false
    }
    ...
    /// for settings that should be persisted to ZZZ's settings file.
    ...
    LI: LspInstaller<BinaryVersion = BinaryVersion> + LspAdapter,
{
    ...
    fn get_language_server_command(
        ...
        async move {
            ...
            //      worktree 3: no gopls found in PATH -> fallback to ZZZ installation
            //
...
impl LanguageScope {
    ...
    ///
    /// By default, ZZZ treats as paragraph and comment prefixes as boundaries.
    ...
    ///
    /// By default, ZZZ treats alphanumeric characters (and '_') as word characters for
