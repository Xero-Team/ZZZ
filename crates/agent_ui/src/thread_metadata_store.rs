...
use agent::{ThreadStore, ZZZ_AGENT_ID};
...
pub struct ArchivedGitWorktree {
    ...
    /// the worktree was in detached HEAD state, which isn't supported in ZZZ, but
    /// could happen if the user made a detached one outside of ZZZ.
...
impl ThreadMetadataDb {
    ...
    pub async fn save(&self, row: ThreadMetadata) -> anyhow::Result<()> {
        ...
        let agent_id = if row.agent_id.as_ref() == ZZZ_AGENT_ID.as_ref() {
            None
        } else {
...
mod tests {
    ...
    async fn test_migrate_thread_metadata_migrates_only_missing_threads(cx: &mut TestAppContext) {
        ...
        assert!(
            list.iter()
                .all(|metadata| metadata.agent_id.as_ref() == agent::ZZZ_AGENT_ID.as_ref())
        );
    ...
    fn test_thread_worktree_paths_full_add_then_remove_cycle() {
        ...
        //   1. Start with linked worktree selectric → zzz
        //   2. Add cloud
        //   3. Remove zzz

        let mut paths = make_worktree_paths(&[("/projects/zzz", "/worktrees/selectric/zzz")]);
        ...
        assert_eq!(
            ...
            &PathList::new(&[
                Path::new("/worktrees/selectric/zzz"),
                Path::new("/projects/cloud"),
            ])
        );
        assert_eq!(
            paths.main_worktree_path_list(),
            &PathList::new(&[Path::new("/projects/zzz"), Path::new("/projects/cloud"),])
        );

        // Step 3: remove zzz
        paths.remove_main_path(Path::new("/projects/zzz"));
    ...
    fn test_thread_worktree_paths_add_is_idempotent() {
        let mut paths = make_worktree_paths(&[("/projects/zzz", "/projects/zzz")]);

        paths.add_path(Path::new("/projects/zzz"), Path::new("/projects/zzz"));
    ...
    fn test_thread_worktree_paths_from_path_lists_preserves_association() {
        ...
        let main = PathList::new(&[Path::new("/projects/zzz"), Path::new("/projects/cloud")]);
        ...
        assert!(pairs.contains(&(
            PathBuf::from("/projects/zzz"),
            PathBuf::from("/worktrees/selectric/zzz")
        )));
        assert!(pairs.contains(&(
    ...
    fn test_thread_worktree_paths_main_deduplicates_linked_worktrees() {
        ...
        let paths = make_worktree_paths(&[
            ("/projects/zzz", "/worktrees/selectric/zzz"),
            ("/projects/zzz", "/worktrees/feature/zzz"),
        ]);
        ...
        assert_eq!(
            ...
            &PathList::new(&[
                Path::new("/worktrees/selectric/zzz"),
                Path::new("/worktrees/feature/zzz"),
            ])
        );
        assert_eq!(
            paths.main_worktree_path_list(),
            &PathList::new(&[Path::new("/projects/zzz"), Path::new("/projects/zzz"),])
        );
    }
    ...
    fn test_thread_worktree_paths_mismatched_lengths_returns_error() {
        ...
        let main = PathList::new(&[Path::new("/projects/zzz")]);
