...
mod tests {
        ...
        model_specs
            ...
            .map(|(provider, name)| {
                ...
                ModelInfo {
                    ...
                    icon: IconOrSvg::Icon(IconName::ZZZAgent),
                    is_favorite,
                }
            })
            .collect()
    ...
    fn test_exact_match(cx: &mut TestAppContext) {
        let models = create_models(vec![
            ("zzz", "Claude 3.7 Sonnet"),
            ("zzz", "Claude 3.7 Sonnet Thinking"),
            ("zzz", "gpt-5"),
            ("zzz", "gpt-5-mini"),
    ...
    fn test_fuzzy_match(cx: &mut TestAppContext) {
        ...
        // In the case below, `zzz/gpt-5-mini` and `openai/gpt-5-mini` have identical
        // similarity scores, but `zzz/gpt-5-mini` was higher in the models list,
        // so it should appear first in the results.
        let results = matcher.fuzzy_search("mini");
        assert_models_eq(results, vec!["zzz/gpt-5-mini", "openai/gpt-5-mini"]);
        ...
        assert_models_eq(results, vec!["zzz/Claude 3.7 Sonnet Thinking"]);
    }

    #[gpui::test]
    fn test_recommended_models_also_appear_in_other(_cx: &mut TestAppContext) {
        let recommended_models = create_models(vec![("zzz", "claude")]);
        let all_models = create_models(vec![
            ("zzz", "claude"), // Should also appear in "other"
            ("zzz", "gemini"),
            ("copilot", "o3"),
        ]);
        ...
        assert_models_eq(
            actual_all_models,
            vec!["zzz/claude", "zzz/gemini", "copilot/o3"],
        );
    }
    ...
    fn test_models_from_different_providers(_cx: &mut TestAppContext) {
        ...
        assert_models_eq(
            actual_all_models,
            vec!["zzz/claude", "zzz/gemini", "copilot/claude"],
        );
    }
    ...
    fn test_favorites_section_appears_when_favorites_exist(_cx: &mut TestAppContext) {
        ...
        let all_models = create_models_with_favorites(
            vec![("zzz", "claude"), ("zzz", "gemini"), ("openai", "gpt-4")],
            vec![("zzz", "gemini")],
        );
        ...
        assert_models_eq(grouped_models.favorites, vec!["zzz/gemini"]);
    }

    #[gpui::test]
    fn test_no_favorites_section_when_no_favorites(_cx: &mut TestAppContext) {
        let recommended_models = create_models(vec![("zzz", "claude")]);
        let all_models = create_models(vec![("zzz", "claude"), ("zzz", "gemini")]);
    ...
    fn test_models_have_correct_actions(_cx: &mut TestAppContext) {
        let recommended_models =
            create_models_with_favorites(vec![("zzz", "claude")], vec![("zzz", "claude")]);
        let all_models = create_models_with_favorites(
            vec![("zzz", "claude"), ("zzz", "gemini"), ("openai", "gpt-4")],
            vec![("zzz", "claude")],
        );
        ...
        for entry in &entries {
            if let LanguageModelPickerEntry::Model(info) = entry {
                if info.model.telemetry_id() == "zzz/claude" {
                    assert!(info.is_favorite, "zzz/claude should be a favorite");
                } else {
                    assert!(
                        !info.is_favorite,
    ...
    fn test_favorites_appear_in_other_sections(_cx: &mut TestAppContext) {
        let favorites = vec![("zzz", "gemini"), ("openai", "gpt-4")];

        let recommended_models =
            create_models_with_favorites(vec![("zzz", "claude")], favorites.clone());
        ...
        assert_models_eq(grouped_models.favorites, vec!["zzz/gemini", "openai/gpt-4"]);
        assert_models_eq(grouped_models.recommended, vec!["zzz/claude"]);
        assert_models_eq(
            grouped_models.all.values().flatten().cloned().collect(),
            vec!["zzz/claude", "zzz/gemini", "openai/gpt-4", "openai/gpt-3.5"],
        );
    }
}
