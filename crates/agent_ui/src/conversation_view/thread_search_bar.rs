...
use ui::{
    ButtonStyle, Color, IconButton, IconButtonShape, IconName, IntoElement, Label, LabelSize,
...
impl ThreadSearchBar {
    ...
    fn build_query(&self, cx: &App) -> (Option<Arc<SearchQuery>>, Option<SharedString>) {
        ...
        match result {
            Ok(query) => (Some(Arc::new(query)), None),
    ...
    pub(super) fn update_matches(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some(thread_view) = self.thread_view.upgrade() else {
            self.clear_results(cx);
            cx.notify();
            return;
        };
        ...
        for (entry_ix, entry) in thread.entries().iter().enumerate() {
            match entry {
                ...
                _ => {
                    for markdown in collect_markdowns(entry_ix, entry, &thread_view, cx) {
                        let source: SharedString = markdown.read(cx).source().into();
            ...
            match target {
                ...
                ScannedTarget::Markdown {
                    ...
                    markdown.update(cx, |markdown, cx| {
                        markdown.set_search_highlights(ranges, None, cx);
                    });
                }
            }
        }
    ...
    fn activate_match(
        ...
        let entry_ix = mat.entry_ix;
        let source_index = mat.source_range.start;
        let target = mat.target.clone();
        ...
        let bar_row = h_flex()
            ...
            .child(
                input_box(border_color)
                    .flex_1()
                    .min_w_32()
                    .child(h_flex().flex_1().min_w_0().py_1().child(render_query_input(
                        &self.query_editor,
                        in_error_state,
                        cx,
                    )))
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        h_flex().ml_1().min_w(rems(2.5)).child(
...
fn collect_markdowns(
    ...
    match entry {
        ...
        AgentThreadEntry::ToolCall(tool_call) => {
            ...
            if thread_view.is_tool_call_output_visible(tool_call) {
                out.extend(
                    tool_call
                        .content
                        .iter()
        ...
        AgentThreadEntry::CompletedPlan(entries) => {
            out.extend(entries.iter().map(|entry| entry.content.clone()));
        }
    }
    out
}
