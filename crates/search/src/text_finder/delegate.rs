//! The text finder is a minimal modal interface to the project search. It is
//! targeted towards search for exploration. It can also be used as a filter
//! step to the project search.
...
use gpui::{
    ...
    FocusHandle, HighlightStyle, Modifiers, StyledText, Task, TextStyle, WeakEntity, prelude::*,
};
use i18n::tr;
...
impl InProgressSearch {
    fn take_disconnected(&mut self) -> Option<SearchResults<SearchResult>> {
        if matches!(self, InProgressSearch::Disconnected(_)) {
            ...
            match placeholder {
                InProgressSearch::Disconnected(results_stream) => Some(results_stream),
...
pub(crate) async fn matches_to_multibuffer(
    ...
    for buffer_id in buffer_order_in_text_finder {
        ...
        let (buffer, ranges) = by_buffer.remove(&buffer_id).expect("ranges inserted");
    ...
    fn searchbar_trailer(
        ...
        Some(
            h_flex()
                ...
                .child(
                    IconButton::new(
                        "text-finder-case-sensitive",
                        SearchOption::CaseSensitive.icon(),
                    )
                    .shape(IconButtonShape::Square)
                    .toggle_state(self.search_options.contains(SearchOptions::CASE_SENSITIVE))
                    .tooltip(Tooltip::text(SearchOption::CaseSensitive.label(cx)))
                    .on_click(cx.listener(|this, _, _window, cx| {
                        this.delegate
                            .search_options
                ...
                .child(
                    IconButton::new("text-finder-regex", SearchOption::Regex.icon())
                        .shape(IconButtonShape::Square)
                        .toggle_state(self.search_options.contains(SearchOptions::REGEX))
                        .tooltip(Tooltip::text(SearchOption::Regex.label(cx)))
                        .on_click(cx.listener(|this, _, _window, cx| {
                            this.delegate.search_options.toggle(SearchOptions::REGEX);
                            cx.notify();
                        })),
                )
                .child(
                    IconButton::new("text-finder-include-ignored", IconName::Sliders)
                        .shape(IconButtonShape::Square)
                        .toggle_state(self.search_options.contains(SearchOptions::INCLUDE_IGNORED))
                        .tooltip(Tooltip::text(SearchOption::IncludeIgnored.label(cx)))
                        .on_click(cx.listener(|this, _, _window, cx| {
                            this.delegate
                                .search_options
...
/// Byte range around the match to render: a bounded slice of the matched line
/// so rendering never scales with line length.
...
mod tests {
    ...
    async fn test_finder_caps_matches_on_long_line(cx: &mut TestAppContext) {
        ...
        let workspace = window
            ...
            .expect("test window contains a workspace");

        let delegate = window
            .update(cx, |_mw, window, cx| {
                workspace.update(cx, |workspace, cx| Delegate::new(workspace, window, cx))
            })
            .expect("test window remains available while creating the delegate")
            .await;
        let picker = window
            .update(cx, |_mw, window, cx| {
                cx.new(|cx| Picker::list(delegate, window, cx))
            })
            .expect("test window remains available while creating the picker");

        window
            .update(cx, |_mw, window, cx| {
                picker.update(cx, |picker, cx| picker.set_query("return", window, cx))
            })
            .expect("test window remains available while setting the query");
    ...
    async fn test_builds_one_match_per_occurrence(cx: &mut TestAppContext) {
        ...
        .expect("the static text query is valid");
