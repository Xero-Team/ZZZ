...
// Output example in the root of ZZZ project:
// ```sh
// ❯ cargo pkgid zzz
// path+file:///absolute/path/to/project/zzz/crates/zzz#0.131.0
// ```
...
mod tests {
    ...
    fn test_target_info_from_metadata() {
        for (input, absolute_path, expected) in [
            (
                r#"{"packages":[{"id":"path+file:///absolute/path/to/project/zzz/crates/zzz#0.131.0","manifest_path":"/path/to/zzz/Cargo.toml","targets":[{"name":"zzz","kind":["bin"],"src_path":"/path/to/zzz/src/main.rs"}]}]}"#,
                "/path/to/zzz/src/main.rs",
                Some((
                    Some(TargetInfo {
                        package_name: "zzz".into(),
                        target_name: "zzz".into(),
                        required_features: Vec::new(),
                        target_kind: TargetKind::Bin,
                    }),
                    Arc::from("/path/to/zzz".as_ref()),
                )),
            ),
            (
