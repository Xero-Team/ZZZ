...
const rfc3339_delimiter = /[ T]/;
const rfc3339_time = /([01][0-9]|2[0-3]):([0-5][0-9])(:([0-5][0-9]|60)([.][0-9]+)?)?/;
const rfc3339_offset = /(Z)|([+-]([01][0-9]|2[0-3]):[0-5][0-9])/;
...
module.exports = grammar({
  ...
  rules: {
    document: ($) => seq(repeat(choice($.pair, newline)), repeat(choice($.table, $.table_array_element))),

    comment: ($) => token(prec(-1, seq("#", repeat(/[^\x00-\x08\x0a-\x1f\x7f]/)))),

    table: ($) => seq("[", choice($.dotted_key, $._key), "]", $._line_ending_or_eof, repeat(choice($.pair, newline))),

    table_array_element: ($) =>
      seq("[[", choice($.dotted_key, $._key), "]]", $._line_ending_or_eof, repeat(choice($.pair, newline))),
    ...
    _basic_string: ($) =>
      seq(
        '"',
        repeat(choice(token.immediate(repeat1(/[^\x00-\x08\x0a-\x1f\x22\x5c\x7f]/)), $.escape_sequence)),
        token.immediate('"'),
      ),
    _multiline_basic_string: ($) =>
      seq(
        ...
        repeat(
          choice(
            token.immediate(repeat1(/[^\x00-\x08\x0a-\x1f\x22\x5c\x7f]/)),
            $._multiline_basic_string_content,
    ...
    escape_sequence: ($) => token.immediate(/\\([befnrt"\\]|x[0-9a-fA-F]{2}|u[0-9a-fA-F]{4}|U[0-9a-fA-F]{8})/),
    _escape_line_ending: ($) => token.immediate(seq(/\\/, newline)),
    _literal_string: ($) =>
      seq("'", optional(token.immediate(repeat1(/[^\x00-\x08\x0a-\x1f\x27\x7f]/))), token.immediate("'")),
    _multiline_literal_string: ($) =>
      seq(
        "'''",
        repeat(
          choice(
            token.immediate(repeat1(/[^\x00-\x08\x0a-\x1f\x27\x7f]/)),
            $._multiline_literal_string_content,
            token.immediate(newline),
          ),
        ),
    ...
    integer: ($) => choice(decimal_integer, hexadecimal_integer, octal_integer, binary_integer),
    ...
    float: ($) =>
      choice(
        token(
          seq(
            decimal_integer,
            choice(float_fractional_part, seq(optional(float_fractional_part), float_exponent_part)),
          ),
        ),
        /[+-]?(inf|nan)/,
      ),
    ...
    offset_date_time: ($) => token(seq(rfc3339_date, rfc3339_delimiter, rfc3339_time, rfc3339_offset)),
    local_date_time: ($) => token(seq(rfc3339_date, rfc3339_delimiter, rfc3339_time)),
    ...
    inline_table: ($) =>
      seq(
        "{",
        optional(seq(alias($._inline_pair, $.pair), repeat(seq(",", alias($._inline_pair, $.pair))), optional(","))),
        "}",
      ),
  },
});
