...
impl DapLocator for PythonLocator {
    ...
    async fn create_scenario(
        ...
        let valid_program = build_config.command.starts_with("$ZZZ_")
        ...
        let program_position = mod_name
            .is_none()
            .then(|| {
                let zzz_file = VariableName::File.template_value_with_whitespace();
                build_config.args.iter().position(|arg| *arg == zzz_file)
            })
            .flatten();
