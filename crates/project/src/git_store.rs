...
/// Represents the handler of a git cat-file --batch process within ZZZ
    ...
    pub fn set_active_repo_for_worktree(
        ...
        let Some(repo_id) = self
            ...
            .filter(|repo| {
                ...
                // The folder opened in ZZZ isn't necessarily the repo root; it may be
...
impl RepositorySnapshot {
    ...
    /// created from.
    ///
    /// For example, if you had both `~/code/zzz` and `~/code/worktrees/zzz-2`,
    /// then `~/code/zzz` is the main worktree and `~/code/worktrees/zzz-2` is a linked worktree.
    ///
    ...
    pub fn access(&mut self, _cx: &App) -> oneshot::Receiver<GitAccess> {
        self.send_job("access", None, move |git_repo, _cx| async move {
            match git_repo {
                ...
                // that's running the ZZZ remote may not own the `.git/`
...
/// - Otherwise (e.g. `zzz.git` for a bare clone), `common_dir` itself is
...
/// Returns the repository identity only when the host can interpret the path style.
///
/// Callers that rely on `std::path` use this to keep foreign path styles unchanged.
...
/// Returns whether `git_dir` is a Git submodule's git directory.
///
/// Submodules keep their git directory below the superproject's
/// `<superproject>/.git/modules/<name>`. Unlike linked worktrees, their
/// identity is their own working directory.
