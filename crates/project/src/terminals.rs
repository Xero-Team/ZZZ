...
use terminal::{
    TaskState, TaskStatus, Terminal, TerminalBuilder, insert_zzz_terminal_env,
...
impl Project {
    ...
    /// In remote projects: opens in ZZZ's launch directory (bypasses SSH).
    ...
    pub fn create_local_terminal(
        ...
        let working_directory = if self.remote_client.is_some() {
            // Remote project: don't use remote paths, let shell use ZZZ's cwd
            None
        } else {
...
fn create_remote_shell(
    ...
    insert_zzz_terminal_env(&mut env, &release_channel::AppVersion::global(cx));
