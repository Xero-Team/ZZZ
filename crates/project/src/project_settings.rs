...
pub struct NodeBinarySettings {
    ...
    /// The path to the npm binary ZZZ should use (defaults to `.path/../npm`).
    pub npm_path: Option<String>,
    /// If enabled, ZZZ will download its own copy of Node.
...
pub struct GitSettings {
    ...
    /// to `false` to keep author emails local.
    ///
    /// Default: true
...
pub struct DiagnosticsSettings {
    ...
    /// Settings for using LSP pull diagnostics mechanism in ZZZ.
...
/// SettingsObserver observers changes to .ZZZ/{settings, task}.json files in local worktrees
/// (or the equivalent protobuf messages from upstream) and updates local settings
/// and sends notifications downstream.
/// In ssh mode it also monitors ~/.config/ZZZ/{settings, task}.json and sends the content
/// upstream.
                        ...
                        Some(
                            async move {
                                ...
                                if abs_path.ends_with(local_vscode_tasks_file_relative_path().as_std_path()) {
                                    ...
                                    let zzz_tasks = TaskTemplates::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode tasks into ZZZ ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&zzz_tasks).with_context(|| {
                                        format!(
                                            "serializing ZZZ tasks into JSON, file {abs_path:?}"
                                        )
                                    })
                                    ...
                                    let zzz_tasks = DebugTaskFile::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode debug tasks into ZZZ ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&zzz_tasks).with_context(|| {
                                        format!(
