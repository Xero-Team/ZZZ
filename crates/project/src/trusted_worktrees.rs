//! A module, responsible for managing the trust logic in ZZZ.
//!
...
//!
//! ZZZ does not consider invisible, `worktree.is_visible() == false` worktrees in ZZZ, as those are programmatically created inside ZZZ for internal needs, e.g. a tmp dir for `keymap_editor.rs` needs.
//!
//!
//! Path rust hierarchy.
//!
//! ZZZ has multiple layers of trust, based on the requests and [`PathTrust`] enum variants.
//! From the least to the most trusted level:
//!
//! * "single file worktree"
//!
//! After opening an empty ZZZ it's possible to open just a file, same as after opening a directory in ZZZ it's possible to open a file outside of this directory.
//! Usual scenario for both cases is opening ZZZ's settings.json file via `zzz: open settings file` command: that starts a language server for a new file open, which originates from a newly created, single file worktree.
//!
//! Spawning a language server is potentially dangerous, and ZZZ needs to restrict that by default.
//! Each single file worktree requires a separate trust permission, unless a more global level is trusted.
//!
//! * "directory worktree"
//!
//! If a directory is open in ZZZ, it's a full worktree which may spawn multiple language servers associated with it.
...
impl TrustedWorktreesStore {
    ...
    /// Requires ZZZ's restart to take proper effect.
