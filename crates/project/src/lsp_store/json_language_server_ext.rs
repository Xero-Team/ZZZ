...
/// In our case, we provide the uris for actions on server startup under the `zzz://schemas/action/{normalize_action_name}` scheme.
        ...
        for states in local.language_servers.values() {
            let json_server = match states {
                super::LanguageServerState::Running {
                    adapter, server, ..
                } if adapter.adapter.is_primary_zzz_json_schema_adapter() => server.clone(),
                _ => continue,
            };
