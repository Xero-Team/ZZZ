        ...
        for (new_diagnostic, entry) in diagnostics {
            ...
            if range.start == range.end {
                ...
                let line_end = snapshot.clip_point_utf16(
                    Unclipped(PointUtf16::new(start.0.row, u32::MAX)),
                    Bias::Left,
                );
...
impl LspStore {
    ...
    async fn regenerate_completion_labels(
        ...
        let mut new_label = match completion_item {
            Some(completion_item) => {
                ...
                // the resolvable properties ZZZ advertises. Regenerate labels here to handle this.
    ...
    pub(crate) fn set_language_server_statuses_from_proto(
        ...
        self.language_server_statuses = language_servers
            ...
            .map(|(server, server_capabilities)| {
                ...
                if let Some(ref language_name) = language_name {
                    self.try_register_remote_adapter_locally(&name, language_name);
                }
    ...
    async fn handle_start_language_server(
        ...
        lsp_store.update(&mut cx, |lsp_store, cx| {
            ...
            if let Some(ref language_name) = language_name {
                lsp_store.try_register_remote_adapter_locally(&server_name, language_name);
            }
    ...
    pub fn install_default_prettier(&mut self, cx: &mut Context<Self>) {
        if let Some(local) = self.as_local() {
            local.prettier_store.update(cx, |prettier_store, cx| {
                prettier_store.install_default_prettier(None, std::iter::empty(), true, cx);
            });
        }
    }
    ...
    pub fn pull_document_diagnostics_for_buffer_edit(
        ...
        for server_id in languages_servers {
            drop(self.pull_document_diagnostics_for_server(server_id, Some(buffer_id), cx));
        }
    }
            ...
            match reg.method.as_str() {
                ...
                "textDocument/diagnostic" => {
                        ...
                        .transpose()?
                    {
                        ...
                        drop(self.pull_document_diagnostics_for_server(server_id, None, cx));
                    }
                }
...
async fn normalize_lsp_relative_path(
    ...
    while let Some(component) = components.next() {
        ...
        exact_path.push(RelPath::unix(component)?);
        ...
        let Some(matched_path) = case_insensitive_match else {
            ...
            return Ok(exact_path.into());
        };
        normalized_path = matched_path;
    }
    Ok(normalized_path.into())
}
...
mod tests {
    ...
    fn lsp_range(start_line: u32, end_line: u32) -> lsp::Range {
        lsp::Range::new(
