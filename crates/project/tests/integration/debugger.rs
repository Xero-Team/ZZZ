mod go_locator {
    ...
    async fn test_create_scenario_for_go_build(_: &mut TestAppContext) {
        ...
        let task = TaskTemplate {
            ...
            env: Default::default(),
            cwd: Some("${ZZZ_WORKTREE_ROOT}".into()),
...
mod python_locator {
    ...
    async fn test_python_locator() {
        ...
        let build_task = TaskTemplate {
            label: "run module '$ZZZ_FILE'".into(),
            command: "$ZZZ_CUSTOM_PYTHON_ACTIVE_ZZZ_TOOLCHAIN".into(),
            args: vec!["-m".into(), "$ZZZ_CUSTOM_PYTHON_MODULE_NAME".into()],
            env: {
                let mut env = HashMap::default();
        ...
        let expected_scenario = DebugScenario {
            ...
            config: json!({
                "request": "launch",
                "python": "$ZZZ_CUSTOM_PYTHON_ACTIVE_ZZZ_TOOLCHAIN",
                "args": [],
                "cwd": "$ZZZ_WORKTREE_ROOT",
                "module": "$ZZZ_CUSTOM_PYTHON_MODULE_NAME",
