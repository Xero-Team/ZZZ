...
pub static ZZZ_DISABLE_STAFF: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("ZZZ_DISABLE_STAFF").is_ok_and(|value| !value.is_empty() && value != "0")
});
...
///
/// Feature flags are enabled for members of ZZZ staff by default. To disable this behavior
/// so you can test flags being disabled, set ZZZ_DISABLE_STAFF=1 in your environment,
/// which will force ZZZ to treat the current user as non-staff.
pub trait FeatureFlag {
    ...
    /// Returns whether this feature flag is enabled for ZZZ staff.
