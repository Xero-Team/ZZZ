...
use crate::{FeatureFlag, FeatureFlagValue, FeatureFlagsSettings, ZZZ_DISABLE_STAFF};
...
impl FeatureFlagStore {
    ...
    pub fn try_flag_value<T: FeatureFlag>(&self, cx: &App) -> Option<T::Value> {
        ...
        if (cfg!(debug_assertions) || self.staff) && !*ZZZ_DISABLE_STAFF && T::enabled_for_staff() {
            return Some(T::Value::on_variant());
        }
    ...
    pub fn has_flag_default<T: FeatureFlag>() -> bool {
        ...
        cfg!(debug_assertions) && T::enabled_for_staff() && !*ZZZ_DISABLE_STAFF
    }
}
