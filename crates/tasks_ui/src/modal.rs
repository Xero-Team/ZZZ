...
pub use zzz_actions::{Rerun, Spawn};
...
impl TasksModalDelegate {
    fn new(
        ...
        }) = &task_overrides
        {
            ...
            Arc::from(tr(
                cx,
                "tasks_ui.modal.placeholder.default",
...
impl PickerDelegate for TasksModalDelegate {
                            ...
                            IconButton::new("delete", IconName::Close)
                                ...
                                .tooltip(|_, cx| {
                                    Tooltip::simple(
                                        tr(
                                            cx,
                                            "tasks_ui.delete_from_recent_tasks",
    ...
    fn render_footer(
        ...
        {
            Some((
                tr(cx, "tasks_ui.modal.rerun_last_task", "Rerun Last Task"),
                    ...
                    if (current_modifiers.alt || self.matches.is_empty()) && !self.prompt.is_empty()
                    {
                        ...
                        this.child({
                            let spawn_oneshot_label = if current_modifiers.secondary() {
                                tr(
                                    cx,
                                    "tasks_ui.modal.spawn_oneshot_without_history",
                                    "Spawn Oneshot Without History",
                                )
                            } else {
                                tr(cx, "tasks_ui.modal.spawn_oneshot", "Spawn Oneshot")
                            };
                        ...
                        this.child({
                            let label = if is_recent_selected {
                                tr(
                                    cx,
                                    "tasks_ui.modal.rerun_without_history",
                                    "Rerun Without History",
                                )
                            } else {
                                tr(
                                    cx,
                                    "tasks_ui.modal.spawn_without_history",
        ...
        fs.insert_tree(
            ...
            json!({
                ".zzz": {
                    "tasks.json": r#"[
                        {
                            "label": "hello from $ZZZ_FILE:$ZZZ_ROW:$ZZZ_COLUMN",
                            "command": "echo",
                            "args": ["hello", "from", "$ZZZ_FILE", ":", "$ZZZ_ROW", ":", "$ZZZ_COLUMN"]
                        },
                        {
                            "label": "opened now: $ZZZ_WORKTREE_ROOT",
                            "command": "echo",
                            "args": ["opened", "now:", "$ZZZ_WORKTREE_ROOT"]
                        }
                    ]"#,
                },
            ...
            language_registry.add(Arc::new(
                ...
                .with_context_provider(Some(Arc::new(
                    ContextProviderWithTasks::new(TaskTemplates(vec![
                        ...
                        TaskTemplate {
                            label: "TypeScript task from file $ZZZ_FILE".to_string(),
                            command: "npm run build".to_string(),
                            ..TaskTemplate::default()
                        },
                        TaskTemplate {
                            label: "Another task from file $ZZZ_FILE".to_string(),
