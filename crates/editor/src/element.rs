use crate::{
    ...
    editor_settings::{
        CurrentLineHighlight, DocumentColorsRenderMode, DoubleClickInMultibuffer, LineNumberScale,
...
impl EditorElement {
    ...
    fn layout_line_numbers(
        ...
        let segments = gutter
            ...
            .flat_map(|(ix, row_info)| {
                ...
                if !(gutter.snapshot.number_deleted_lines
    ...
    fn gutter_strip_width(line_height: Pixels, cx: &App) -> Pixels {
        match EditorSettings::get_global(cx).gutter.git_gutter_width {
            Some(width) => px(width),
            None => (Self::DEFAULT_STRIP_WIDTH_RATIO * line_height).floor(),
        }
    }
    ...
    fn shape_line_number(
        ...
        let line_number_size = DefiniteLength::Fraction(match line_number_scale {
            LineNumberScale::Default => 1.0,
            LineNumberScale::Small => 0.875,
            LineNumberScale::XSmall => 0.75,
        });
    ...
    fn test_deleted_marker_base_width() {
        ...
        for line_height in [22.0, 40.0] {
            let widths = [1.0, 2.0, 3.0, 6.0].map(|width| {
                EditorElement::deleted_marker_base_width(Some(width), px(line_height))
            });
            assert!(
        ...
        assert_eq!(
            EditorElement::deleted_marker_base_width(Some(0.275 * 40.0), px(40.0)),
            px(14.0),
        );
    }
