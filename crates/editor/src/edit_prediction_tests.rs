...
#[gpui::test]
async fn test_edit_prediction_jump_disabled_for_non_zzz_providers(cx: &mut gpui::TestAppContext) {
    init_test(cx, |_| {});

    let mut cx = EditorTestContext::new(cx).await;
    let provider = cx.new(|_| FakeNonZZZEditPredictionDelegate::default());
    assign_editor_completion_provider_non_zzz(provider.clone(), &mut cx);
    ...
    // For non-ZZZ providers, there should be no move completion (jump functionality disabled)
    cx.editor(|editor, _, _| {
        if let Some(completion_state) = &editor.active_edit_prediction {
            ...
            match &completion_state.completion {
                EditPrediction::Edit { .. } => {
                    // This is expected for non-ZZZ providers
                }
                EditPrediction::MoveWithin { .. } | EditPrediction::MoveOutside { .. } => {
                    panic!(
                        "Non-ZZZ providers should not show Move predictions (jump functionality)"
                    );
                }
            }
        }
...
fn propose_edits_non_zzz<T: ToOffset>(
    provider: &Entity<FakeNonZZZEditPredictionDelegate>,
    edits: Vec<(Range<T>, &str)>,
    cx: &mut EditorTestContext,
) {
    let snapshot = cx.buffer_snapshot();
...
fn assign_editor_completion_provider_non_zzz(
    provider: Entity<FakeNonZZZEditPredictionDelegate>,
    cx: &mut EditorTestContext,
) {
    cx.update_editor(|editor, window, cx| {
...
impl EditPredictionDelegate for FakeEditPredictionDelegate {
    ...
    fn icons(&self, _cx: &gpui::App) -> EditPredictionIconSet {
        EditPredictionIconSet::new(IconName::ZZZPredict)
    }
...
#[derive(Default, Clone)]
pub struct FakeNonZZZEditPredictionDelegate {
    pub completion: Option<edit_prediction_types::EditPrediction>,
}

impl FakeNonZZZEditPredictionDelegate {
    pub fn set_edit_prediction(
        &mut self,
        completion: Option<edit_prediction_types::EditPrediction>,
    ) {
...
impl EditPredictionDelegate for FakeNonZZZEditPredictionDelegate {
    fn name() -> &'static str {
        "fake-non-zzz-provider"
    }
