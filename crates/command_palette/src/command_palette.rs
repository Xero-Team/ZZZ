...
impl PickerDelegate for CommandPaletteDelegate {
    ...
    fn placeholder_text(&self, _window: &mut Window, cx: &mut App) -> Arc<str> {
        tr(
            cx,
            "command_palette.placeholder.execute_command",
            "Execute a command...",
        )
        .into()
    }
    ...
    fn render_footer(
        ...
        let keybinding_buttons = if keybind.has_binding(window) {
            Button::new(
                ...
                tr(
                    cx,
                    "command_palette.change_keybinding",
                    "Change Keybinding...",
                ),
            )
            .key_binding(
            ...
            Button::new(
                "add",
                tr(cx, "command_palette.add_keybinding", "Add Keybinding..."),
            )
            .key_binding(
                KeyBinding::for_action_in(&menu::SecondaryConfirm, focus_handle, cx)
                    .map(|kb| kb.size(rems_from_px(12.))),
        ...
        Some(
            h_flex()
                ...
                .child(
                    Button::new("run-action", tr(cx, "menu.run", "Run"))
                        .key_binding(
