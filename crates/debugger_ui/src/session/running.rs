...
use task::{
    ...
    TaskContext, ZZZDebugConfig, substitute_variables_in_str,
};
    ...
    pub(crate) fn substitute_variables_in_config(
        ...
        match config {
            ...
            serde_json::Value::String(s) => {
                ...
                if s.starts_with("\"$ZZZ_") && s.ends_with('"') {
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            if config_is_valid {
                ...
                let scenario = dap_registry
                    .adapter(&adapter)
                    .with_context(|| anyhow!("{}: is not a valid adapter name", adapter))?.config_from_zzz_format(zzz_config)
                    .await?;
                ...
                anyhow::bail!("ZZZ cannot determine how to run this debug scenario. `build` field was not provided and Debug Adapter won't accept provided configuration because: {e}");
            };
