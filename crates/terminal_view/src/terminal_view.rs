...
impl TerminalView {
    ...
    /// Handles editor copy so the application menu can validate `Edit > Copy`
    ...
    /// Handles editor paste so the application menu can validate `Edit > Paste`
    /// when a terminal is focused.
    fn editor_paste(
        &mut self,
...
mod tests {
    ...
    async fn draw_standalone_terminal(
        ...
        let terminal = cx.new(|cx| {
            ...
            .expect("display-only terminal should initialize")
            .subscribe(cx)
        });
        terminal.update(cx, |terminal, cx| {
