        ...
        let instance = if options.allow_binary_download {
            ...
            match ManagedNodeRuntime::install_if_needed(&state.http).await {
                Ok(instance) => {
                    log::log!(
                        log_level,
                        "using ZZZ managed Node.js at {} since {}",
            ...
            match result {
                Ok(output) => {
                    if output.status.success() {
                        ...
                        log::warn!(
                            "ZZZ managed Node.js binary at {} failed check with output: {:?}",
                            node_binary.display(),
                            output
                        );
                        false
                ...
                Err(err) => {
                    log::warn!(
                        "ZZZ managed Node.js binary at {} failed check, so re-downloading it. \
                        Error: {}",
                        node_binary.display(),
                        err
                    );
