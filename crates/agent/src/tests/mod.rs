use super::*;
use acp_thread::{PermissionOptions, UserMessageId};
...
use gpui::{App, AppContext, AsyncApp, Entity, Task, TestAppContext, UpdateGlobal};
use indoc::indoc;
use language_model::{
...
async fn test_edit_file_tool_allow_still_prompts_for_local_settings(cx: &mut TestAppContext) {
    ...
    // Editing a file inside .ZZZ/ should still prompt even with global default: allow,
    ...
    let _task = cx.update(|cx| {
        tool.run(
            ToolInput::resolved(crate::EditFileToolInput {
                ...
                path: "root/.ZZZ/settings.json".into(),
                mode: crate::EditFileMode::Edit,
                content: None,
                edits: None,
            }),
