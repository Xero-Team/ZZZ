...
pub const DOCS_URL: &str = "https://codeberg.org/ZZZEditor/ZZZ/wiki";
                            ...
                            .child(
                                h_flex()
                                    ...
                                    .child({
                                        Button::new(
                                            "finish_setup",
                                            tr(cx, "onboarding.finish_setup", "Finish Setup"),
                                        )
...
pub async fn handle_import_vscode_settings(
    ...
    let vscode_settings =
        match settings::VsCodeSettings::load_user_settings(source, fs.clone()).await {
            ...
            Err(err) => {
                zlog::error!("{err:?}");
                let load_failed_message = cx
                    .update(|_, cx| {
                        tr(
                            cx,
                            "onboarding.settings_import.load_failed",
                            "Could not find or load a {} settings file",
                        )
                        .replace("{}", &source.to_string())
                    })
                    .unwrap_or_else(|_| format!("Could not find or load a {source} settings file"));
                let ok_button = cx
                    .update(|_, cx| tr(cx, "onboarding.common.ok", "Ok"))
    ...
    if !skip_prompt {
        let overwrite_warning = cx
            .update(|_, cx| {
                tr(
                    cx,
                    "onboarding.settings_import.overwrite_warning",
                    "Importing {} settings may overwrite your existing settings. Will import settings from {}",
                )
                .replacen("{}", &vscode_settings.source.to_string(), 1)
                .replacen(
                    "{}",
        ...
        let cancel_button = cx
            .update(|_, cx| tr(cx, "onboarding.common.cancel", "Cancel"))
            .unwrap_or_else(|_| "Cancel".to_owned());
        let prompt = cx.prompt(
            gpui::PromptLevel::Warning,
            &overwrite_warning,
            None,
            &[ok_button.as_str(), cancel_button.as_str()],
        );
        ...
        .update_in(cx, |workspace, _, cx| match result {
            Ok(_) => {
                let confirmation_toast = StatusToast::new(
                    tr(
                        cx,
                        "onboarding.settings_import.success",
            ...
            Err(_) => {
                ...
                let error_toast = StatusToast::new(
                    tr(
                        cx,
                        "onboarding.settings_import.failed",
                    ...
                    |this, _| {
                        ...
                        .action(open_log_label.clone(), |window, cx| {
