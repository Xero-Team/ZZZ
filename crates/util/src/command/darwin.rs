...
mod tests {
    ...
    fn test_bare_program_resolved_via_custom_path() {
        ...
        let link_path = temp_dir.path().join("zzz-test-echo");
        ...
        smol::block_on(async {
            let output = Command::new("zzz-test-echo")
    ...
    fn test_bare_program_preserves_argv0_when_resolved_via_custom_path() {
        ...
        let link_path = temp_dir.path().join("zzz-test-sh");
        std::os::unix::fs::symlink("/bin/sh", &link_path).expect("failed to create symlink");

        let custom_path = temp_dir.path().to_string_lossy().into_owned();

        smol::block_on(async {
            let output = Command::new("zzz-test-sh")
                .args(["-c", "printf %s \"$0\""])
                .env("PATH", &custom_path)
                .output()
                .await
            ...
            assert!(output.status.success());
            assert_eq!(output.stdout, b"zzz-test-sh");
        });
    }
    ...
    fn test_bare_program_with_custom_path_falls_back_when_not_found() {
        smol::block_on(async {
            let result = Command::new("zzz-nonexistent-binary-xyz")
