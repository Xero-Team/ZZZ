...
async fn test_remote_archive_git_operations_are_supported(
    ...
    cx.update(|cx| {
        repository.update(cx, |repository, _| {
            repository.update_ref("refs/zzz-tests/archive-checkpoint".to_string(), head_sha)
        })
    })
    .await
    ...
    cx.update(|cx| {
        repository.update(cx, |repository, _| {
            repository.delete_ref("refs/zzz-tests/archive-checkpoint".to_string())
        })
    })
    .await
