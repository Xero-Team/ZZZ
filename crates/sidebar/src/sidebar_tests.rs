...
/// same `RemoteConnectionIdentity`, matching how ZZZ treats multiple
...
fn save_thread_metadata(
    ...
    cx.update(|cx| {
        ...
        let metadata = ThreadMetadata {
            ...
            agent_id: agent::ZZZ_AGENT_ID.clone(),
            title,
            updated_at,
            created_at,
            interacted_at,
        ...
        s.contents.entries = vec![
            ...
            ListEntry::Thread(ThreadEntry {
                metadata: ThreadMetadata {
                    ...
                    agent_id: AgentId::new("zzz-agent"),
...
// `[/cloud, /worktrees/zzz/wt_a/zzz]`, where:
//   - `/cloud` is a standalone git repo (main == folder).
//   - `/worktrees/zzz/wt_a/zzz` is a linked worktree of `/zzz`.
//
// Once git scans complete the project group key is
// `[/cloud, /zzz]` — the main paths of the two roots. A thread
// created in this workspace is written with
// `main=[/cloud, /zzz], folder=[/cloud, /worktrees/zzz/wt_a/zzz]`
...
// `[/cloud, /worktrees/zzz/wt_a/zzz]` ≠ `[/cloud, /zzz]`. The three
// lookups in `rebuild_contents` all miss:
//
//   1. `entries_for_main_worktree_path([/cloud, /zzz])` — the
//      thread's stored main doesn't equal the group key.
//   2. `entries_for_path([/cloud, /zzz])` — the thread's folder paths
...
//      thread's folder `/worktrees/zzz/wt_a/zzz` doesn't match.
//
...
async fn test_sidebar_keeps_multi_root_thread_with_stale_main_paths(cx: &mut TestAppContext) {
    ...
    // Separate /zzz repo whose linked worktree will form the second
    // workspace root. /zzz itself is NOT opened as a workspace root.
    fs.insert_tree(
        "/zzz",
        serde_json::json!({
            ".git": {},
    ...
    .await;
    fs.insert_tree(
        "/worktrees/zzz/wt_a/zzz",
        serde_json::json!({
            ".git": "gitdir: /zzz/.git/worktrees/wt_a",
            "src": {},
        }),
    )
    .await;
    fs.add_linked_worktree_for_repo(
        ...
        false,
        git::repository::Worktree {
            path: std::path::PathBuf::from("/worktrees/zzz/wt_a/zzz"),
    ...
    let project = project::Project::test(
        fs.clone(),
        ["/cloud".as_ref(), "/worktrees/zzz/wt_a/zzz".as_ref()],
        cx,
    )
    .await;
    project.update(cx, |p, cx| p.git_scans_complete(cx)).await;
    ...
    let expected_main_paths = PathList::new(&[PathBuf::from("/cloud"), PathBuf::from("/zzz")]);
    assert_eq!(
        group_key.path_list(),
        &expected_main_paths,
        "expected the multi-root workspace's project group key to normalize to \
         [/cloud, /zzz] (main of the standalone repo + main of the linked worktree)"
    );

    let folder_paths = PathList::new(&[
        PathBuf::from("/cloud"),
        PathBuf::from("/worktrees/zzz/wt_a/zzz"),
    ]);
    let workspace_root_paths = workspace.read_with(cx, |ws, cx| PathList::new(&ws.root_paths(cx)));
    assert_eq!(
        workspace_root_paths, folder_paths,
        "expected the workspace's root paths to equal [/cloud, /worktrees/zzz/wt_a/zzz]"
    );
...
async fn test_archive_last_worktree_thread_removes_workspace(cx: &mut TestAppContext) {
    ...
    .await;
    agent_ui::test_support::record_zzz_created_worktree(
        fs.as_ref(),
...
async fn test_remote_archive_thread_with_active_connection(
    ...
    // linked worktree across two remote projects the way ZZZ does in
