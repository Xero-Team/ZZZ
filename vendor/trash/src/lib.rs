...
impl TrashContext {
    ...
    pub fn delete_with_info<T: AsRef<Path>>(&self, path: T) -> Result<TrashItem, Error> {
        self.delete_all_with_info(&[path])?
            .pop()
            .ok_or(Error::Unknown {
                description: "delete_with_info did not return trash item information".into(),
            })
    }
        ...
        self.delete_all_canonicalized(full_paths, true)
            .and_then(|items| {
                items.ok_or(Error::Unknown {
                    description: "delete_all_with_info did not return trash item information"
                        .into(),
                })
            })
    }
...
pub mod os_limited {
    ...
    use super::{Error, TrashItem, TrashItemMetadata, platform};
