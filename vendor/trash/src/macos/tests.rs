use crate::{
    ...
    macos::{DeleteMethod, TrashContextExtMacos, percent_encode},
    restore_all,
...
fn test_delete_with_info_ns_file_manager() {
    ...
    match trash.delete_with_info(&path) {
        Ok(trash_item) => {
            ...
            assert_eq!(
                trash_item.original_parent,
                path.parent().expect("Should have parent").as_os_str()
            );
...
fn test_trash_and_restore_roundtrip_finder() {
    ...
    let trash_item = trash
        .delete_with_info(&path)
        .expect("Should be able to delete the file");
    assert!(!path.exists());

    restore_all(vec![trash_item]).expect("Should successfully restore the trash item");

    let file_contents =
        std::fs::read_to_string(&path).expect("Should be able to read file contents");
    assert!(path.exists());
...
fn test_restore_all_restore_collision_file_manager() {
    ...
    match restore_all(vec![trash_item.clone()]) {
        Err(super::Error::RestoreCollision {
            path: collision_path,
            remaining_items,
        }) => {
...
fn test_restore_all_missing_trash_item() {
    ...
    let id = std::env::current_dir()
        .expect("Should be able to get current directory")
        .join(get_unique_name());
    let name: OsString = id
        .file_name()
        .expect("Should be able to get the file name")
        .into();
    let original_parent = id
        .parent()
        .expect("Should be able to get parent")
        .to_path_buf();
    let time_deleted = 0;
    ...
    match restore_all(vec![trash_item]) {
        Err(Error::Unknown { description }) => {
            assert_eq!(description, format!("Trash item not found at {:?}", id))
        }
