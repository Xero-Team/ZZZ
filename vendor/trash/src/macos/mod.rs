...
use crate::{Error, TrashContext, TrashItem, fs_error, into_unknown};
...
fn delete_using_file_mgr<P: AsRef<Path>>(
    full_paths: &[P],
    with_info: bool,
) -> Result<Option<Vec<TrashItem>>, Error> {
...
fn delete_using_finder<P: AsRef<Path>>(
    full_paths: &[P],
    with_info: bool,
) -> Result<Option<Vec<TrashItem>>, Error> {
    ...
    I: IntoIterator<Item = TrashItem>,
{
    ...
    while let Some(item) = iter.next() {
        ...
        if !std::fs::exists(&item.id).map_err(into_unknown)? {
            return Err(Error::Unknown {
                description: format!("Trash item not found at {:?}", item.id),
            });
        }
        ...
        std::fs::create_dir_all(&item.original_parent)
            .map_err(|error| fs_error(&original_path, error))?;
